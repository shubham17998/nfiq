...
public class MatchPattern extends MindTct implements IMatchPattern {
	...
	private static volatile MatchPattern instance;
    public static MatchPattern getInstance() {
        if (instance == null) {
	...
	**************************************************************************
	...
	#cat:            first pixel pair match the specified pixel pair.
	   Input:
	      firstPixelValue - first pixel value of pair
	      secondPixelValue - second pixel value of pair
	   Output:
	      oPossible - list of matching feature_patterns[] indices
	      oPossibleMatch    - number of matches
	   Return Code:
	      oPossibleMatch    - number of matches
	*************************************************************************/
	public int matchFirstPair(int firstPixelValue, int secondPixelValue, AtomicIntegerArray oPossible, AtomicInteger oPossibleMatch) {
		...
		for (int i = 0; i < ILfs.NFEATURES; i++)
		{
			...
			if ((firstPixelValue == getGlobals().getFeaturePatterns()[i].first[0]) && 
				(secondPixelValue == getGlobals().getFeaturePatterns()[i].first[1]))
			{
	...
	/*************************************************************************
	**************************************************************************
	#cat: matchSecondPair - Determines which of the passed feature_patterns[] have
	#cat:            their second pixel pair match the specified pixel pair.
	   Input:
	      firstPixelValue - first pixel value of pair
	      secondPixelValue - second pixel value of pair
	      oPossible - list of potentially-matching feature_patterns[] indices
	      oPossibleMatch    - number of potential matches
	   Output:
	      oPossible - list of matching feature_patterns[] indices
	      oPossibleMatch    - number of matches
	   Return Code:
	      oPossibleMatch    - number of matches
	*************************************************************************/
	public int matchSecondPair(int firstPixelValue, int secondPixelValue, AtomicIntegerArray oPossible, AtomicInteger oPossibleMatch) {
		...
		for (int i = 0; i < currentPossibleMatches; i++)
		{
			...
			if ((firstPixelValue == getGlobals().getFeaturePatterns()[oPossible.get(i)].second[0]) && 
				(secondPixelValue == getGlobals().getFeaturePatterns()[oPossible.get(i)].second[1]))
			{
				/* Store feature as a oPossible match. */
	...
	**************************************************************************
	...
	#cat:            their third pixel pair match the specified pixel pair.
	   ...
	   Return Code:
	      oPossibleMatch    		- number of matches
	*************************************************************************/
	public int matchThirdPair(int firstPixelValue, int secondPixelValue, AtomicIntegerArray oPossible, AtomicInteger oPossibleMatch) {
		int currentPossibleMatches;
		...
		for (int i = 0; i < currentPossibleMatches; i++)
		{
			...
			if ((firstPixelValue == getGlobals().getFeaturePatterns()[oPossible.get(i)].third[0]) && 
				(secondPixelValue == getGlobals().getFeaturePatterns()[oPossible.get(i)].third[1]))
			{
				/* Store feature as a oPossible match. */
				oPossible.set(oPossibleMatch.get(), oPossible.get(i));
				/* Bump number of stored possibilities. */
	...
	/*************************************************************************
	**************************************************************************
	#cat: skipRepeatedHorizontalPair - Takes the location of two pixel in
	...
	#cat:            itself or the image region is exhausted.
	   Input:
	      currentXPixelIndex    	- current x-coord of starting pixel pair
	      currentRightXPixelIndex   - right edge of the image region
	      binarizedImageData 	- array of image
	      currentTopPixel 		- pointer to current top pixel in pair
	      currentBottomPixel 	- pointer to current bottom pixel in pair
	      imageWidth    		- width (in pixels) of image
	      imageHeight    		- height (in pixels) of image
	   Output:
	      currentXPixelIndex    - x-coord of where rightward skip terminated
	...
	*************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: skipRepeatedVerticalPair - Takes the location of two pixel in
	...
	#cat:            itself or the image region is exhausted.
	   Input:
	      currentYPixelIndex    	- current y-coord of starting pixel pair
	      currentBottomYPixelIndex  - bottom of the image region
	      binarizedImageData 		- array of image
	      currentLeftPixel 			- pointer to current left pixel in pair
	      currentRightPixel 		- pointer to current right pixel in pair
	      imageWidth    			- width (in pixels) of image
	      imageHeight    			- height (in pixels) of image
	   Output:
