...
public class Block extends MindTct implements IBlock {
    ...
    **************************************************************************
    ...
    #cat:       images.  The strategy used here is simple, but others are possible.
       Input:
          imageWidth        - width (in pixels) of the orginal input image
          imageHeight        - height (in pixels) of the orginal input image
       ...
       Output:
          blockOffsets      - points to the list of pixel offsets to the origin of
                      each block in the "padded" input image
          oImageWidth        - the number of horizontal blocks in the input image
          oImageHeight        - the number of vertical blocks in the input image
       Return Code:
    ...
    **************************************************************************/
    public AtomicIntegerArray blockOffsets(AtomicInteger ret, AtomicInteger oImageWidth, AtomicInteger oImageHeight, 
		...
		int bx, by, blockImageWidth, blockImageHeight, bi, blockImageSize, blockRowStart, blockRowSize, offset, lastbw, lastbh;
		int pad2, paddedImageWidth, paddedImageHeight;

		/* Test if unpadded image is smaller than a single block */
		if ((imageWidth < blockSize) || (imageHeight < blockSize))
		{
			LOGGER.error(String.format("ERROR : block_offsets : image must be at least %d by %d in size\n", blockSize, blockSize));
    ...
    #cat:             processing.
       Input:
          blockOffset - byte offset into the padded input image to the origin of
    ...
    **************************************************************************
    **************************************************************************/
    public int lowContrastBlock(int blockOffset, int blockSize, 
		int[] paddedImageData, int paddedImageWidth, int paddedImageHeight, LfsParams lfsparms) {
		int[] pixTable = new int[ILfs.IMG_6BIT_PIX_LIMIT];
		int numOfPix;
		int pi;
		int currentPaddedImageIndex, paddedImageIndex;
		int delta;
		double tdbl;
		int prctMin = 0, prctMax = 0, prctThresh, pixSum, found;

		numOfPix = blockSize * blockSize;
		Arrays.fill(pixTable, 0);

		tdbl = (lfsparms.getPercentileMinMax() / 100.0) * (double)(numOfPix - 1);
    ...
    /*************************************************************************
    **************************************************************************
    #cat: findValidBlock - Take a Direction Map, Low Contrast Map,
    ...
    #cat:             code of FOUND.  Otherwise, a code of NOT_FOUND is returned.
       Input:
          directionMap    - map of blocks containing directional ridge flows
          lowContrastMap - map of blocks flagged as LOW CONTRAST
          startX        - X-block coord where search starts in maps
          startY        - Y-block coord where search starts in maps
          mappedImageWidth        - number of blocks horizontally in the maps
          mappedImageHeight        - number of blocks vertically in the maps
          xIncr    - X-block increment to direct search
          yIncr    - Y-block increment to direct search
       Output:
          nbrDir   - valid direction found
          nbrX     - X-block coord where valid direction found
          nbrY     - Y-block coord where valid direction found
       Return Code:
    ...
    **************************************************************************/
    public int findValidBlock(AtomicInteger nbrDir, AtomicInteger nbrX, AtomicInteger nbrY,
		...
		int startX, int startY, int mappedImageWidth, int mappedImageHeight,
		int xIncr, int yIncr) {
		int xPixel, yPixel, dir;
    ...
    **************************************************************************
    ...
    #cat:             the specified value.
       Input:
          oMap       - map of blocks to be modified
          mappedImageWidth        - number of blocks horizontally in the map
          mappedImageHeight        - number of blocks vertically in the map
          marginValue - value to be assigned to the perimeter blocks
       Output:
          oMap       - resulting map
    **************************************************************************/
