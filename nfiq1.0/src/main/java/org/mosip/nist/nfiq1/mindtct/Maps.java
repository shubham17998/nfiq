...
public class Maps extends MindTct implements IMaps {
	...
	private static volatile Maps instance;
    ...
    public static Maps getInstance(int mappedImageWidth, int mappedImageHeight) {
        if (instance == null) {
            synchronized (Maps.class) {
                if (instance == null) {
    ...
    public static Maps getInstance(AtomicIntegerArray directionMap, AtomicIntegerArray lowContrastMap, AtomicIntegerArray lowFlowMap,
	...
	private AtomicIntegerArray directionMap, lowContrastMap, lowFlowMap, highCurveMap;
	...
	private AtomicInteger mappedImageWidth, mappedImageHeight;
	...
	/*************************************************************************
	**************************************************************************
	#cat: genImageMaps - Computes a set of image maps based on Version 2
	...
	#cat:            generate maps for an arbitrarily sized, non-square, image.
	   Input:
	      ...
	      paddedImageHeight        - padded height (in pixels) of the input image
	      dirToRad   - lookup table for converting integer directions
	   ...
	   Output:
	      //oDirectionMap     - points to the created Direction Map(Take from Map Object Get Method)
	      //oLowContrastMap    - points to the created Low Contrast Map(Take from Map Object Get Method)
	      //oLowFlowMap    - points to the Low Ridge Flow Map(Take from Map Object Get Method)
	      //oHighCurvatureMap    - points to the High Curvature Map(Take from Map Object Get Method)
	      //omw       - width (in blocks) of the maps(Take from Get Map Object Method)
	      //omh       - height (in blocks) of the maps (Take from Map Object Get Method)
	   Return Code:
	...
	**************************************************************************/
	public int genImageMaps(int [] paddedImagedata, final int paddedImageWidth, final int paddedImageHeight, 
		...
		AtomicInteger mappedImageWidth = new AtomicInteger(0), mappedImageHeight = new AtomicInteger(0);
		...
		blockOffsets = getBlock().blockOffsets(ret, mappedImageWidth, mappedImageHeight, 
			imageWidth, imageHeight, dftGrids.getPad(), lfsParams.getBlockSize());
		...
		ret.set(initialiseMaps(getDirectionMap(), getLowContrastMap(), getLowFlowMap(), blockOffsets, 
			getMappedImageWidth().get(), getMappedImageHeight().get(), 
		...
		ret.set(interpolateDirectionMap(getDirectionMap(), getLowContrastMap(), getMappedImageWidth().get(), getMappedImageHeight().get(), lfsParams));
		if (ret.get() != 0)
		{
			return ret.get();
		}
		...
		getBlock().setMarginBlocks(getDirectionMap(), getMappedImageWidth().get(), getMappedImageHeight().get(), ILfs.INVALID_DIR);
		...
		ret.set(generateHighCurveMap(getHighCurveMap(), getDirectionMap(), getMappedImageWidth().get(), getMappedImageHeight().get(), lfsParams));
		if (ret.get() != ILfs.FALSE)
		{
			return ret.get();
		}
	...
	/*************************************************************************
	**************************************************************************
	#cat: initialiseMaps - Creates an initial Direction Map from the given
	...
	#cat:             INVALID in the Direction Map.
	   Input:
	      blockOffsets   - offsets to the pixel origin of each block in the padded image
	      mappedImageWidth        - number of blocks horizontally in the padded input image
	      mappedImageHeight        - number of blocks vertically in the padded input image
	      paddedImagedata     - padded input image data (8 bits [0..256) grayscale)
	      paddedImageWidth        - width (in pixels) of the padded input image
	      paddedImageHeight        - height (in pixels) of the padded input image
	      dftWaves  - structure containing the DFT wave forms
	      dftGrids  - structure containing the rotated pixel grid offsets
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oDirectionMap     - points to the newly created Direction Map
	      oLowContrastMap    - points to the newly created Low Contrast Map
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int initialiseMaps(AtomicIntegerArray oDirectionMap, AtomicIntegerArray oLowContrastMap, 
		AtomicIntegerArray oLowFlowMap, AtomicIntegerArray blockOffsets, final int mappedImageWidth, final int mappedImageHeight, 
		...
		int nStats;
		AtomicInteger ret = new AtomicInteger(0); // return code
		int dftOffset, xminLimit, xmaxLimit, yminLimit, ymaxLimit, winX, winY, lowContrastOffset;
		...
		for (bi = 0; bi < bSize; bi++)
		{
			...
			dftOffset = blockOffsets.get(bi) - (lfsParams.getWindowOffset() * paddedImageWidth) - lfsParams.getWindowOffset();
			...
			if (isShowLogs())
				LOGGER.info(String.format("   MAP BLOCK %2d (%2d, %2d) ", bi, bi % mappedImageWidth, bi / mappedImageWidth));

			/* If block is low contrast ... */
			ret.set(getBlock().lowContrastBlock(lowContrastOffset, lfsParams.getWindowSize(), 
				paddedImagedata, paddedImageWidth, paddedImageHeight, lfsParams));
			if (ret.get() != ILfs.FALSE)
			{
				/* If system error ... */
				if (ret.get() < ILfs.FALSE)
				...
				if (isShowLogs())
					LOGGER.info(String.format("LOW CONTRAST"));
			...
			else
			{
				...
				/* Compute DFT powers */
				ret.set(getDft().dftDirPowers(powers, paddedImagedata, lowContrastOffset, 
					paddedImageWidth, paddedImageHeight, dftWaves, dftGrids));
				if (ret.get() != ILfs.FALSE)
				{
					/* Free memory allocated to this point. */
					oDirectionMap = null;
				...
				ret.set(getDft ().getDftPowerStats(wis, powmaxs, powmaxDirs, pownorms, powers, 
					ILfs.TRUE, dftWaves.getNWaves(), dftGrids.getNoOfGrids()));
				if (ret.get() != ILfs.FALSE)
				{
					/* Free memory allocated to this point. */
					oDirectionMap = null;
				...
				{  
					...
					LOGGER.info(String.format( "         wis[%d] %d %12.3f %2d %9.3f %12.3f\n",
					_w, wis.get(_w)+1, 
	...
	/*************************************************************************
	**************************************************************************
	#cat: interpolateDirectionMap - Take a Direction Map and Low Contrast
	...
	#cat:             and perimeter of the fingerprint in the image.
	   Input:
	      ...
	      oLowContrastMap - map of blocks flagged as LOW CONTRAST
	      mappedImageWidth        - number of blocks horizontally in the maps
	      mappedImageHeight        - number of blocks vertically in the maps
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oDirectionMap - contains the newly interpolated results
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int interpolateDirectionMap(AtomicIntegerArray oDirectionMap, AtomicIntegerArray oLowContrastMap, 
		...
		int newDir;
		AtomicInteger northDir = new AtomicInteger(0), eastDir = new AtomicInteger(0), 
				southDir = new AtomicInteger(0), westDir = new AtomicInteger(0);
		int northDist = 0, eastDist = 0, southDist = 0, westDist = 0, totalDist;
		int northFound, eastFound, southFound, westFound, totalFound;
		int northDelta = 0, eastDelta = 0, southDelta = 0, westDelta = 0, totalDelta;
		AtomicInteger nbrX = new AtomicInteger(0), nbrY = new AtomicInteger(0);
		AtomicIntegerArray oMap;
		int dptrIndex = 0, cptrIndex = 0, mptrIndex = 0;
		double avrDir;
		...
		for (int y = 0; y < mappedImageHeight; y++)
		{
			for (int x = 0; x < mappedImageWidth; x++)
			{
				...
				if ((oLowContrastMap.get(cptrIndex) == 0) && (oDirectionMap.get(dptrIndex) == ILfs.INVALID_DIR))
				{
					...
					/* Find north neighbor. */
					if ((northFound = getBlock().findValidBlock(northDir, nbrX, nbrY, 
						oDirectionMap, oLowContrastMap, x, y, 
						mappedImageWidth, mappedImageHeight, 0, -1)) == ILfs.FOUND)
					{
					...
					/* Find east neighbor. */
					if ((eastFound = getBlock().findValidBlock(eastDir, nbrX, nbrY, 
						oDirectionMap, oLowContrastMap, x, y, 
						mappedImageWidth, mappedImageHeight, 1, 0)) == ILfs.FOUND)
					{
					...
					/* Find south neighbor. */
					if ((southFound = getBlock().findValidBlock(southDir, nbrX, nbrY, 
						oDirectionMap, oLowContrastMap, x, y, mappedImageWidth, 
						mappedImageHeight, 0, 1)) == ILfs.FOUND)
					{
					...
					/* Find west neighbor. */
					if ((westFound = getBlock().findValidBlock(westDir, nbrX, nbrY, 
						oDirectionMap, oLowContrastMap, x, y, mappedImageWidth, 
						mappedImageHeight, -1, 0)) == ILfs.FOUND)
					{
					...
					if (totalFound >= lfsParams.getMinInterpolateNbrs())
					{
						...
						if (isShowLogs())
							LOGGER.info(String.format("Block %d,%d INTERP numnbs=%d newdir=%d\n", x, y , totalFound ,newDir));
	...
	**************************************************************************/
	public int morphMapWithTF(AtomicIntegerArray tfMap, final LfsParams lfsParams) {
		int [] cimage, mimage;
		int cptrIndex, mptrIndex;
		int i;
		final int mappedImageWidth = getMappedImageWidth().get(), mappedImageHeight = getMappedImageHeight().get(); 

		if (isShowLogs())
			LOGGER.info(String.format("morphMapWithTF Started (%d, %d)\n", mappedImageWidth , mappedImageHeight));
	...
	**************************************************************************
	...
	#cat:            values in maps to be directly accessed via pixel addresses.
	   Input:
	      ...
	      imageWidth        	- the width (in pixels) of the corresponding image
	      imageHeight        	- the height (in pixels) of the corresponding image
	      inputBlockImageMap    - input block image map
	      mapWidth      - the width (in blocks) of the map
	      mapHeight    	- the height (in blocks) of the map
	   ...
	   Return Code:
	      ret 		- Zero     - successful completion
	      			- Negative - system error
	**************************************************************************/
	public int pixelizeMap(AtomicIntegerArray oMap, int imageWidth, int imageHeight, AtomicIntegerArray inputBlockImageMap, 
		final int mapWidth, final int mapHeight, final int blockSize) {
		AtomicInteger ret = new AtomicInteger(0);
		AtomicIntegerArray blockOffsets = null;
		AtomicInteger oBlockOffsetWidth = new AtomicInteger (0), oBlockOffsetHeight = new AtomicInteger (0);
		int mapIndex;
		int blockOffsetsIndex = 0, mapCurrentIndex = 0;

		//Assigned before only // pmap

		blockOffsets = getBlock().blockOffsets(ret, oBlockOffsetWidth, oBlockOffsetHeight, 
			imageWidth, imageHeight, 0, blockSize);
		if (ret.get() != ILfs.FALSE)
		{
			oMap = null;
			return ret.get();
		...
		if ((oBlockOffsetWidth.get() != mapWidth) || 
			(oBlockOffsetHeight.get() != mapHeight))
		{
	...
	**************************************************************************
	...
	#cat:               them by analyzing the direction of adjacent neighbors.
	   Input:
	      oDirectionMap 		- vector of integer block values
	      oLowContrastMap 		- vector of integer block values
	      //mappedImageWidth    - width (in blocks) of the map
	      //mappedImageHeight   - height (in blocks) of the map
	      dirToRad   		- lookup table for converting integer directions
	      lfsParams 		- parameters and thresholds for controlling LFS
	   Output:
	      oDirectionMap      - vector of smoothed input values
	**************************************************************************/
	public void smoothDirectionMap(AtomicIntegerArray oDirectionMap, AtomicIntegerArray oLowContrastMap, 
		...
		AtomicInteger oAverageDir = new AtomicInteger (0), oValid = new AtomicInteger (0);
		AtomicReference<Double> oDirectionStrength = new AtomicReference<Double> ();
					...
					if (oDirectionStrength.get() >= lfsParams.getDirStrengthMin())
					{
						...
						if (oDirectionMap.get(directionMapIndex) != ILfs.INVALID_DIR)
						{
							...
							if (oValid.get() >= lfsParams.getRmvValidNbrMin())
							{
								if (isShowLogs())
								{
									LOGGER.info(String.format("   SMOOTH DIRECTION BLOCK %2d (%2d, %2d)\n", mappedXIndex+(mappedYIndex*mappedImageWidth), mappedXIndex, mappedYIndex));
									LOGGER.info(String.format("      Average NBR :   %2d %6.3f %d\n", oAverageDir.get(), oDirectionStrength.get(), oValid.get()));
									LOGGER.info(String.format("      1. Valid NBR (%d >= %d)\n", oValid.get(), lfsParams.getRmvValidNbrMin()));
									LOGGER.info(String.format("      Valid Direction = %d\n", oDirectionMap.get(directionMapIndex)));
									LOGGER.info(String.format("      Smoothed Direction = %d\n", oAverageDir.get()));
								}
						...
						else
						{
							...
							if (oValid.get() >= lfsParams.getSmoothValidNbrMin())
							{
								if (isShowLogs())
								{
									LOGGER.info(String.format("   SMOOTH DIRECTION BLOCK %2d (%2d, %2d)\n", mappedXIndex+(mappedYIndex*mappedImageWidth), mappedXIndex, mappedYIndex));
									LOGGER.info(String.format("      Average NBR :   %2d %6.3f %d\n", oAverageDir.get(), oDirectionStrength.get(), oValid.get()));
									LOGGER.info(String.format("      2. Invalid NBR (%d >= %d)\n", oValid.get(), lfsParams.getSmoothValidNbrMin()));
									LOGGER.info(String.format("      Invalid Direction = %d\n", oDirectionMap.get(directionMapIndex)));
									LOGGER.info(String.format("      Smoothed Direction = %d\n", oAverageDir.get()));
								}
	...
	**************************************************************************
	...
	#cat:            that flags blocks with HIGH CURVATURE.
	   Input:
	      oHighCurvatureMap    - pointer to the High Curvature Map
	      oDirectionMap - map of blocks containing directional ridge flow
	      mappedImageWidth        - the width (in blocks) of the map
	      mappedImageHeight        - the height (in blocks) of the map
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oHighCurvatureMap    - points to the created High Curvature Map
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int generateHighCurveMap(AtomicIntegerArray oHighCurvatureMap, AtomicIntegerArray oDirectionMap, 
		...
		int curvatureMeasure = 0, vorticityMeasure = 0;
		...
		for (int mappedYIndex = 0; mappedYIndex < mappedImageHeight; mappedYIndex++)
		{
			for (int mappedXIndex = 0; mappedXIndex < mappedImageWidth; mappedXIndex++)
			{
				/* Count number of valid neighbors around current block ... */
		         nvalid.set(numValid8Nbrs(oDirectionMap, mappedXIndex, mappedYIndex, 
	        		 mappedImageWidth, mappedImageHeight));		         
		         ...
		         if (nvalid.get() > ILfs.FALSE)
		         {
		        	 ...
		        	 if (oDirectionMap.get(directionMapIndex) == ILfs.INVALID_DIR)
		        	 {
		        		 ...
		        		 if (nvalid.get() >= lfsParams.getVortValidNbrMin())
		        		 {
		                     ...
		                     vorticityMeasure = vorticity(oDirectionMap, mappedXIndex, mappedYIndex, 
		        	 ...
		        	 else
		        	 {
		                 ...
		                 curvatureMeasure = curvature(oDirectionMap, mappedXIndex, mappedYIndex, 
	                		 mappedImageWidth, mappedImageHeight, lfsParams.getNumDirections());
	...
	/*************************************************************************
	**************************************************************************
	#cat: generateInputBlockImageMap - Computes an IMAP, which is a 2D vector of integer directions,
	...
	#cat:            generate an IMAP for arbitrarily sized, non-square, images.
	   ...
	   Output:
	      ret		- Zero     - successful completion
	      			- Negative - system error
	      oMappedImageWidth        - width (in blocks) of the IMAP
	      oMappedImageHeight        - height (in blocks) of the IMAP
	   Return Code:
	      optr      - points to the created IMAP
	**************************************************************************/
	public AtomicIntegerArray generateInputBlockImageMap(AtomicInteger ret, AtomicInteger oMappedImageWidth, AtomicInteger oMappedImageHeight, 
			int[] paddedImagedata, final int paddedImageWidth, final int paddedImageHeight, final DirToRad dirToRad, final DftWaves dftWaves, 
			final RotGrids dftGrids, final LfsParams lfsParams) {
		...
		blockOffsets = getBlock().blockOffsets(ret, mappedImageWidth, mappedImageHeight, 
			imageWidth, imageHeight, dftGrids.getPad(), dftGrids.getGridWidth ());
		if (ret.get() != ILfs.FALSE)
		{			
			return oInputBlockImageMap;
		}
		...
		/* 2. initial imap */
		oInputBlockImageMap = initialiseInputBlockImageMap(ret, blockOffsets, 
			mappedImageWidth, mappedImageHeight, paddedImagedata, paddedImageWidth, paddedImageHeight, 
			dftWaves, dftGrids, lfsParams);
		if (ret.get() != ILfs.FALSE)
		{
			/* Free memory allocated to this point. */
			blockOffsets = null;
	...
	/*************************************************************************
	**************************************************************************
	#cat: initialiseInputBlockImageMap - Creates an initial IMAP from the given input image.
	...
	#cat:             followed by a smoothing process.
	   ...
	   Output:
      	  ret 		- Zero     - successful completion
	      			- Negative - system error
	   Return Code:
	      inputBlockImageMap      - points to the newly created IMAP
	**************************************************************************/
	@SuppressWarnings("unused")
	public AtomicIntegerArray initialiseInputBlockImageMap(AtomicInteger ret, AtomicIntegerArray blockOffsets, 
			final AtomicInteger mappedImageWidth, final AtomicInteger mappedImageHeight, 
		...
		int bSize, blockDir;
		AtomicIntegerArray wis, powmaxDirs;
		AtomicReferenceArray<Double[]> powers;
		AtomicReferenceArray<Double> powmaxs = null, pownorms = null;
		int nStats;
		...
		{
			/* Compute DFT powers */
			ret.set(getDft().dftDirPowers(powers, paddedImagedata, blockOffsets.get(blockOffsetIndex), 
				paddedImageWidth, paddedImageHeight, dftWaves, dftGrids));
			if (ret.get() != ILfs.FALSE)
			{
				getFree().free(inputBlockImageMap);
			...
			/* wave.  This is dependent on how the primary and secondary */
			/* direction tests work below.                               */
			ret.set(getDft().getDftPowerStats(wis, powmaxs, powmaxDirs, pownorms, powers, 1, 
	...
	/*************************************************************************
	**************************************************************************
	#cat: primaryDirectionTest - Applies the primary set of criteria for selecting
	...
	#cat:                    computed from a block of image data
	   Input:
	      ...
	      powmaxDirs - directions associated with each of the N-1 maximum powers
	...
	public int primaryDirectionTest(AtomicReferenceArray<Double[]> powers, final AtomicIntegerArray wis, 
		...
		for (int statIndex = 0; statIndex < nStats; statIndex++)
		{
				...
				(powers.get(0)[powmaxDirs.get(wis.get(statIndex))] <= lfsParams.getPowmaxMax()))
			{
				...
				if (isShowLogs())
				{
					LOGGER.info(String.format(  "         Selected Wave = %d\n", wis.get(statIndex)+1));
					LOGGER.info(String.format(  "         1. Power Magnitude (%12.3f > %12.3f)\n", powmaxs.get(wis.get(statIndex)), lfsParams.getPowmaxMin()));
					LOGGER.info(String.format( "         2. Norm Power Magnitude (%9.3f > %9.3f)\n", pownorms.get(wis.get(statIndex)), lfsParams.getPownormMin()));
					LOGGER.info(String.format( "         3. Low Freq Wave Magnitude (%12.3f <= %12.3f)\n", powers.get(0)[powmaxDirs.get(wis.get(statIndex))], lfsParams.getPowmaxMax()));
					LOGGER.info(String.format( "         PASSED\n"));
					LOGGER.info(String.format(  "         Selected Direction = %d\n", powmaxDirs.get(wis.get(statIndex))));
				}
		...
		if (isShowLogs())
		{
			LOGGER.info(String.format("         1. Power Magnitude ( > %12.3f)\n", lfsParams.getPowmaxMin()));
			LOGGER.info(String.format("         2. Norm Power Magnitude ( > %9.3f)\n", lfsParams.getPownormMin()));
			LOGGER.info(String.format("         3. Low Freq Wave Magnitude ( <= %12.3f)\n", lfsParams.getPowmaxMax()));
	...
	**************************************************************************/
	public int secondaryForkTest(AtomicReferenceArray<Double[]> powers, final AtomicIntegerArray wis, 
		...
		double forkPownormMin, forkPowThresh;
			...
			(powers.get(0)[powmaxDirs.get(wis.get(0))] <= lfsParams.getPowmaxMax()))
		{
			/* First part passed ... */
		      ...
		      {
		    	  LOGGER.info(String.format( "         Selected Wave = %d\n", wis.get(0)+1));
		    	  LOGGER.info(String.format( "         1. Power Magnitude (%12.3f > %12.3f)\n", powmaxs.get(wis.get(0)), lfsParams.getPowmaxMin()));
		    	  LOGGER.info(String.format( "         2. Norm Power Magnitude (%9.3f >= %9.3f)\n", pownorms.get(wis.get(0)), forkPownormMin));
		    	  LOGGER.info(String.format( "         3. Low Freq Wave Magnitude (%12.3f <= %12.3f)\n", powers.get(0)[powmaxDirs.get(wis.get(0))], lfsParams.getPowmaxMax()));
		      }
			...
			leftDir = (powmaxDirs.get(wis.get(0)) + lfsParams.getNumDirections() - lfsParams.getForkInterval()) % lfsParams.getNumDirections();

			//LOGGER.info(String.format(("         Left = %d, Current = %d, Right = %d\n" + ldir + "----" + powmax_dirs.get(wis.get(0)) + "----" +  rdir)));
				...
				(powers.get(wis.get(0) + 1)[rightDir] > forkPowThresh)))
			{
				if (isShowLogs())
				{
					LOGGER.info(String.format("         4. Left Power Magnitude (%12.3f > %12.3f)\n", powers.get(wis.get(0)+1)[leftDir], forkPowThresh));
					LOGGER.info(String.format("         5. Right Power Magnitude (%12.3f > %12.3f)\n", powers.get(wis.get(0)+1)[rightDir], forkPowThresh));
					LOGGER.info(String.format("         PASSED\n"));
					LOGGER.info(String.format("         Selected Direction = %d\n", powmaxDirs.get(wis.get(0))));
				}
	...
	/*************************************************************************
	**************************************************************************
	#cat: removeInconsistentDirs - Takes a vector of integer directions and removes
	...
	#cat:              a complete pass.
	   Input:
	      oInputBlockImageMap      - vector of IMAP integer directions
	      //mappedImageWidth        - width (in blocks) of the IMAP
	      //mappedImageHeight        - height (in blocks) of the IMAP
	      dirToRad   - lookup table for converting integer directions
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      imap      - vector of pruned input values
	**************************************************************************/
	public void removeInconsistentDirs(AtomicIntegerArray oInputBlockImageMap, final DirToRad dirToRad, final LfsParams lfsParams) {
		int mappedImageXIndex, mappedImageYIndex;
		int nInputBlockImageMapIndex;
		int nRemoved = -1, leftBoxIndex, rightBoxIndex, topBoxIndex, bottomBoxIndex;
		final int mappedImageWidth = getMappedImageWidth().get(), mappedImageHeight = getMappedImageHeight().get(); 

		int numPass = 0;
		if (isShowLogs())
			LOGGER.info(String.format("REMOVE MAP\n"));
		...
		do
		{
			...
			++numPass;
			if (isShowLogs())
				LOGGER.info(String.format("\nREMOVE MAP PASS = %d, %d, %d\n", numPass, oInputBlockImageMap.length(), nRemoved));
			...
			if ((oInputBlockImageMap.get(nInputBlockImageMapIndex) != ILfs.INVALID_DIR) && 
				(removeIMAPDirection(oInputBlockImageMap, mappedImageXIndex, mappedImageYIndex, mappedImageWidth, mappedImageHeight, dirToRad, lfsParams) >= ILfs.TRUE))
			{
				/* Set to INVALID */
			...
			while ((leftBoxIndex >= 0) || (rightBoxIndex < mappedImageWidth) || 
				(topBoxIndex >= 0) || (bottomBoxIndex < mappedImageHeight))
			{
	...
	**************************************************************************
	...
	#cat:                 respect to their adjacent neighbors.
	   Input:
	      leftBoxIndex      - left edge of current concentric square
	      topBoxIndex      - top edge of current concentric square
	      rightBoxIndex      - right edge of current concentric square
	      bottomBoxIndex      - bottom edge of current concentric square
	      oInputBlockImageMap      - vector of IMAP integer directions
	      mappedImageWidth        - width (in blocks) of the IMAP
	      mappedImageHeight        - height (in blocks) of the IMAP
	      dirToRad   - lookup table for converting integer directions
	...
	**************************************************************************/
	public int testTopEdge(final int leftBoxIndex, final int topBoxIndex, final int rightBoxIndex, final int bottomBoxIndex, 
		AtomicIntegerArray oInputBlockImageMap, final int mappedImageWidth, final int mappedImageHeight, DirToRad dirToRad, LfsParams lfsParams) {
		int bx, by, sx, ex;
		...
		for (inputBlockImageMapIndex = inputBlockImageMapCurrentIndex, bx = sx, by = topBoxIndex; inputBlockImageMapIndex <= inputBlockImageMapEdgeIndex; inputBlockImageMapIndex++, bx++)
		{
			...
			if ((oInputBlockImageMap.get(inputBlockImageMapIndex) != ILfs.INVALID_DIR) && 
				(removeIMAPDirection(oInputBlockImageMap, bx, by, mappedImageWidth, mappedImageHeight, 
					dirToRad, lfsParams) >= ILfs.TRUE))
			{
				/* Set to INVALID */
	...
	**************************************************************************
	...
	#cat:                 with respect to their adjacent neighbors.
	   ...
	   Return Code:
	      ...
	      Zero     - direction should NOT be remove from IMAP
	**************************************************************************/
	public int testRightEdge(final int leftBoxIndex, final int topBoxIndex, final int rightBoxIndex, final int bottomBoxIndex, 
		AtomicIntegerArray oInputBlockImageMap, final int mappedImageWidth, final int mappedImageHeight, DirToRad dirToRad, LfsParams lfsParams) {
		int bx, by, sy, ey;
		int inputBlockImageMapIndex, inputBlockImageMapCurrentIndex, inputBlockImageMapEdgeIndex;
		int nRemoved;
		...
		for (inputBlockImageMapIndex = inputBlockImageMapCurrentIndex, bx = rightBoxIndex, by = sy; inputBlockImageMapIndex <= inputBlockImageMapEdgeIndex; inputBlockImageMapIndex += mappedImageWidth, by++)
		{
			/* If valid IMAP direction and test for removal is true ... */
			if ((oInputBlockImageMap.get(inputBlockImageMapIndex) != ILfs.INVALID_DIR) && 
				(removeIMAPDirection(oInputBlockImageMap, bx, by, mappedImageWidth, mappedImageHeight, dirToRad, lfsParams) >=  ILfs.TRUE))
			{
				/* Set to INVALID */
				oInputBlockImageMap.set(inputBlockImageMapIndex, ILfs.INVALID_DIR);
				/* Bump number of removed IMAP directions */
	...
	**************************************************************************
	...
	#cat:               with respect to their adjacent neighbors.
	   ...
	   Return Code:
	      ...
	      Zero     - direction should NOT be remove from IMAP
	**************************************************************************/
	public int testBottomEdge(final int leftBoxIndex, final int topBoxIndex, final int rightBoxIndex, final int bottomBoxIndex, 
		AtomicIntegerArray oInputBlockImageMap, final int mappedImageWidth, final int mappedImageHeight, DirToRad dirToRad, LfsParams lfsParams) {
		int bx, by, sx, ex;
		...
		/* rightmost point in the IMAP ROW (lastx=mappedImageWidth-1), whichever is smaller.  */		
	...
	**************************************************************************
	...
	#cat:                 respect to their adjacent neighbors.
	   ...
	   Return Code:
	      ...
	      Zero     - direction should NOT be remove from IMAP
	**************************************************************************/
	public int testLeftEdge(final int leftBoxIndex, final int topBoxIndex, final int rightBoxIndex, final int bottomBoxIndex, 
		AtomicIntegerArray oInputBlockImageMap, final int mappedImageWidth, final int mappedImageHeight, 
		DirToRad dirToRad, LfsParams lfsParams) {
		int bx, by, sy, ey;
		int inputBlockImageMapIndex;
		int inputBlockImageMapCurrentIndex;
		int inputBlockImageMapEdgeIndex;
	...
	/*************************************************************************
	**************************************************************************
	#cat: removeIMAPDirection - Determines if an IMAP direction should be removed based
	#cat:              on analyzing its adjacent neighbors
	   Input:
	      oInputBlockImageMap      - vector of IMAP integer directions
	      mappedImageXIndex        - IMAP X-coord of the current direction being tested
	      mappedImageYIndex        - IMPA Y-coord of the current direction being tested
	      mappedImageWidth        - width (in blocks) of the IMAP
	      mappedImageHeight        - height (in blocks) of the IMAP
	      dirToRad   - lookup table for converting integer directions
	      lfsParams  - parameters and thresholds for controlling LFS
	   Return Code:
	      ...
	      Zero     - direction should NOT be remove from IMAP
	**************************************************************************/
	public int removeIMAPDirection(AtomicIntegerArray oInputBlockImageMap, 
		final int mappedImageXIndex, final int mappedImageYIndex, final int mappedImageWidth, 
		final int mappedImageHeight, final DirToRad dirToRad, final LfsParams lfsParams) {
		...
		/* number of valid neighbors used in the computation, and  */
		/* the "strength" of the average direction.                */
		average8NbrDir(oAverageDirection, dirStrength, oValid, oInputBlockImageMap, 
			mappedImageXIndex, mappedImageYIndex, mappedImageWidth, mappedImageHeight, dirToRad);
		/* Conduct valid neighbor test (Ex. thresh==3) */
		if (oValid.get() < lfsParams.getRmvValidNbrMin())
		{
			if (isShowLogs())
			{
				LOGGER.info(String.format("      BLOCK %2d (%2d, %2d)\n", mappedImageXIndex+(mappedImageYIndex*mappedImageWidth), mappedImageXIndex, mappedImageYIndex));
				LOGGER.info(String.format("         Average NBR :   %2d %6.3f %d\n", oAverageDirection.get(), dirStrength.get(), oValid.get()));
				LOGGER.info(String.format("         1. Valid NBR (%d < %d)\n", oValid.get(), lfsParams.getRmvValidNbrMin()));
			}
			return (ILfs.TRUE);
		}
		...
		if (dirStrength.get() >= lfsParams.getDirStrengthMin())
		{
			...
			nDistance = Math.abs(oAverageDirection.get() - (oInputBlockImageMap.get((mappedImageYIndex * mappedImageWidth) + mappedImageXIndex)));
			...
			if (nDistance > lfsParams.getDirDistanceMax())
			{
				if (isShowLogs())
				{
					...
					LOGGER.info(String.format("         Average NBR :   %2d %6.3f %d\n", oAverageDirection.get(), dirStrength.get(), oValid.get()));
					LOGGER.info(String.format("         1. Valid NBR (%d < %d)\n", oValid.get(), lfsParams.getRmvValidNbrMin()));
			        LOGGER.info(String.format("         2. Direction Strength (%6.3f >= %6.3f)\n", dirStrength.get(), lfsParams.getDirStrengthMin()));
			        LOGGER.info(String.format("         Current Dir =  %d, Average Dir = %d\n", oInputBlockImageMap.get((mappedImageYIndex*mappedImageWidth)+mappedImageXIndex), oAverageDirection.get()));
			        LOGGER.info(String.format("         3. Direction Distance (%d > %d)\n", nDistance, lfsParams.getDirDistanceMax()));
				}
				return (2);
			}
		}
	...
	/*************************************************************************
	**************************************************************************
	#cat: average8NbrDir - Given an IMAP direction, computes an average
	...
	#cat:                    number of valid direction in the neighborhood.
	   Input:
	      oInputBlockImageMap      - vector of IMAP integer directions
	      mapXIndex        - IMAP X-coord of the current direction
	      my        - IMPA Y-coord of the current direction
	      mappedImageWidth        - width (in blocks) of the IMAP
	      mappedImageHeight        - height (in blocks) of the IMAP
	      dirToRad   - lookup table for converting integer directions
	   Output:
	      oAverageDir    - the average direction computed from neighbors
	      oDirStrength - the strength of the average direction
	      oValid    - the number of valid directions used to compute the
	                  average
	**************************************************************************/
	public void average8NbrDir(AtomicInteger oAverageDir, AtomicReference<Double> oDirStrength, 
		AtomicInteger oValid, AtomicIntegerArray oInputBlockImageMap, final int mapXIndex, final int mapYIndex, 
		final int mappedImageWidth, final int mappedImageHeight, final DirToRad dirToRad) {
		int inputBlockImageMapIndex;
		int eastIndex, westIndex, northIndex, southIndex;
		double cospart, sinpart, pi2, pifactor, theta, avr;
	...
	/*************************************************************************
	**************************************************************************
	#cat: numValid8Nbrs - Given a block in an IMAP, counts the number of
	#cat:                   immediate neighbors that have a valid IMAP direction.
	   Input:
	      oInputBlockImageMap - 2-D vector of directional ridge flows
	      mapXIndex   - horizontal coord of current block in IMAP
	...
	**************************************************************************/
	public int numValid8Nbrs(AtomicIntegerArray oInputBlockImageMap, final int mapXIndex, final int mapYIndex, 
		...
		int eastIndex, westIndex, northIndex, southIndex, nValid;
	...
	/*************************************************************************
	**************************************************************************
	#cat: InputBlockImageMap - Takes a vector of integer directions and smooths them
	#cat:               by analyzing the direction of adjacent neighbors.
	   Input:
	      ...
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oInputBlockImageMap      - vector of smoothed input values
	**************************************************************************/
	public void smoothInputBlockImageMap(AtomicIntegerArray oInputBlockImageMap, final DirToRad dirToRad, final LfsParams lfsParams) {
		int inputBlockImageMapIndex = 0, inputBlockImageMapIndexValue;
		final int mappedImageWidth = getMappedImageWidth().get(), mappedImageHeight = getMappedImageHeight().get(); 
		AtomicInteger averageDir = new AtomicInteger (0), oValid = new AtomicInteger (0);
		AtomicReference<Double> oDirStrength = new AtomicReference<Double>();
		...
		for (int mapYIndex = 0; mapYIndex < mappedImageHeight; mapYIndex++)
		{
			for (int mapXIndex = 0; mapXIndex < mappedImageWidth; mapXIndex++)
			{
				...
				/* number of valid neighbors used in the computation, and  */
				/* the "strength" of the average direction.                */
				average8NbrDir(averageDir, oDirStrength, oValid, oInputBlockImageMap, 
					mapXIndex, mapYIndex, mappedImageWidth, mappedImageHeight, dirToRad);
	...
	**************************************************************************
	...
	#cat:            directions.
	   Input:
	      oInputBlockImageMap      - associated input vector of IMAP directions
	      mappedImageWidth        - the width (in blocks) of the IMAP
	      mappedImageHeight        - the height (in blocks) of the IMAP
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oNMap      - points to the created NMAP
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int genNMap(AtomicIntegerArray oNMap, AtomicIntegerArray oInputBlockImageMap, 
		final int mappedImageWidth, final int mappedImageHeight, final LfsParams lfsParams) {
		int nmapIndex, inputBlockImageMapIndex;
		int nValid, curvatureMeasure, vorticityMeasure;
		...
		for (int mappedImageYIndex = 0; mappedImageYIndex < mappedImageHeight; mappedImageYIndex++)
		{
			...
			{
				/* Count number of valid neighbors around current block ... */
				nValid = numValid8Nbrs(oInputBlockImageMap, mappedImageXIndex, mappedImageYIndex, 
					mappedImageWidth, mappedImageHeight);
	...
	**************************************************************************
	...
	#cat:             among the IMAP neighbors of the given block.
	   Input:
	      oInputBlockImageMap  - 2D vector of ridge flow directions
	      mappedImageXIndex    - horizontal coord of current IMAP block
	      mappedImageXIndex    - vertical coord of current IMAP block
	      mappedImageWidth    - width (in blocks) of the IMAP
	      mappedImageHeight    - height (in blocks) of the IMAP
	...
	**************************************************************************/
	public int vorticity(AtomicIntegerArray oInputBlockImageMap, 
		final int mappedImageXIndex, final int mappedImageYIndex, 
		final int mappedImageWidth, final int mappedImageHeight, final int nDirs) {
		int eastIndex, westIndex, northIndex, southIndex;
		int northwestValue, northValue, northeastValue, eastValue, 
			southeastValue, southValue, southwestValue, westValue;
	...
	**************************************************************************
	...
	#cat:                        between neighboring IMAP blocks.
	   ...
	   Output:
	      oVorticityMeasure - accumulated vorticity among neighbors measured so far
	**************************************************************************/
	public void accumulateNbrVorticity(AtomicInteger oVorticityMeasure, final int dir1, final int dir2, final int nDirs) {
		int dist;
	...
	**************************************************************************
	...
	#cat:             current IMAP direction and its immediate neighbors.
	   Input:
	      oInputBlockImageMap  - 2D vector of ridge flow directions
	      mappedImageXIndex    - horizontal coord of current IMAP block
	      mappedImageYIndex    - vertical coord of current IMAP block
	      mappedImageWidth    - width (in blocks) of the IMAP
	      mappedImageHeight    - height (in blocks) of the IMAP
	      nDirs - number of possible directions in the IMAP
	   Return Code:
	...
	**************************************************************************/
	public int curvature(AtomicIntegerArray oInputBlockImageMap, final int mappedImageXIndex, final int mappedImageYIndex, 
		final int mappedImageWidth, final int mappedImageHeight, final int nDirs) {
		int nInputBlockImageMapIndexValue;
		int eastIndex, westIndex, northIndex, southIndex; 
		int northwestValue, northValue, northeastValue, eastValue, 
			southeastValue, southValue, southwestValue, westValue;
		int nCurvatureMeasure, nDistance;
		...
		nInputBlockImageMapIndexValue = oInputBlockImageMap.get((mappedImageYIndex * mappedImageWidth) + mappedImageXIndex);
