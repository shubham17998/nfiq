...
public class MinutiaHelper extends MindTct implements IMinutia {
	...
	private static volatile MinutiaHelper instance;
    public static MinutiaHelper getInstance() {
        if (instance == null) {
	...
	**************************************************************************
	...
	#cat:            specified maximum number of minutiae to be detected.
	   Input:
	      maxMinutiae - number of minutia to be allocated in list
	   Output:
	...
	**************************************************************************/
	...
	{
		List<Minutia> list = new ArrayList<Minutia>(maxMinutiae);
	...
	/*************************************************************************
	**************************************************************************
	#cat: reallocMinutiae - Reallocates a previously allocated minutia list
	#cat:            extending its allocated length based on the specified
	#cat:            increment.
	   Input:
	      oMinutiae    - previously allocated list of minutiae points
	      incrMinutiae - number of minutia to be allocated in list
	   Output:
	      oMinutiae    - extended list of minutiae points
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: detectMinutiaeV2 - Takes a binary image and its associated
	...
	#cat:            detected in LOW FLOW blocks are set with lower reliability.
	   Input:
	      ...
	      Maps	- object contains all below
	   ...
	   Output:
	      oMinutiae   - points to a list of detected minutia structures
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	public int detectMinutiaeV2(AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		...
		ret.set(map.pixelizeMap(pDirectionMap, mappedImageWidth, mappedImageHeight, map.getDirectionMap(), 
				map.getMappedImageWidth().get(), map.getMappedImageHeight().get(), lfsParams.getBlockSize()));
	...
	/*************************************************************************
	**************************************************************************
	#cat: updateMinutiae - Takes a detected minutia point and (if it is not
	#cat:                determined to already be in the minutiae list) adds it to
	#cat:                the list.
	   Input:
	      oMinutiae   - minutia structure for detected point
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      lfsParams  - parameters and thresholds for controlling LFS
	...
	**************************************************************************/
	public int updateMinutiae(AtomicReference<Minutiae> oMinutiae, Minutia minutia, 
		...
		int minutiaIndex, ret, distanceY, distanceX, deltaDir;
		int qtrNDirs, fullNDirs;
				...
				if (distanceX < lfsParams.getMaxMinutiaDelta())
				{
					...
					if (distanceY < lfsParams.getMaxMinutiaDelta())
					{
						...
						if (oMinutiae.get().getList().get(minutiaIndex).getType() == minutia.getType())
						{
							...
							deltaDir = Math.abs(oMinutiae.get().getList().get(minutiaIndex).getDirection() - minutia.getDirection());
							...
							if (deltaDir <= qtrNDirs)
							{
								...
								if (getContour().searchContour(minutia.getX(), minutia.getY(), 
									lfsParams.getMaxMinutiaDelta(), oMinutiae.get().getList().get(minutiaIndex).getX(), 
									...
									ILfs.SCAN_CLOCKWISE, binarizedImageData, imageWidth, imageHeight) == ILfs.FOUND)
								{
								...
								if (getContour().searchContour(minutia.getX(), minutia.getY(), 
									...
									oMinutiae.get().getList().get(minutiaIndex).getEx(), 
									oMinutiae.get().getList().get(minutiaIndex).getEy(), 
									ILfs.SCAN_COUNTER_CLOCKWISE, binarizedImageData, imageWidth, imageHeight)  == ILfs.FOUND)
								{
									/* Consider the new minutia to be the same as the */
									/* current list minutia, so don't add the new one */
									/* to the list.                                   */
	...
	**************************************************************************
	...
	#cat:                it to the list.
	   Input:
	      oMinutiae   - minutia structure for detected point
	      scanDir  - orientation of scan when minutia was detected
	      directionMapValue   - directional ridge flow of block minutia is in
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oMinutiae   - points to a list of detected oMinutiae structures
	   Return Code:
	      Zero      - minutia added to successfully added to oMinutiae list
	      IGNORE    - minutia is to be ignored (already in the oMinutiae list)
	      Negative  - system error
	**************************************************************************/
	public int updateMinutiaeV2(AtomicReference<Minutiae> oMinutiae, Minutia minutia, 
		final int scanDir, final int directionMapValue, int[] binarizedImageData,
		final int imageWidth, final int imageHeight, final LfsParams lfsParams) {
		int i, ret, dy, dx, deltaDir;
		int qtrNDirs, fullNDirs;
		int mapScanDir;
				...
				if (dx < lfsParams.getMaxMinutiaDelta())
				{
					...
					if (dy < lfsParams.getMaxMinutiaDelta())
					{
						...
						if (oMinutiae.get().getList().get(i).getType() == minutia.getType())
						{
							...
							/* Take minimum of computed inner and outer        */
							/* direction differences.                          */
							deltaDir = Math.abs(oMinutiae.get().getList().get(i).getDirection() - minutia.getDirection());
							deltaDir = Math.min(deltaDir, fullNDirs - deltaDir);
							/* If directional difference is <= 45 degrees... */
							if (deltaDir <= qtrNDirs)
							{
								...
								/* clockwise.                                      */
								/* If new minutia point found on contour...        */
								if (getContour().searchContour(minutia.getX(), minutia.getY(), lfsParams.getMaxMinutiaDelta(), oMinutiae.get().getList().get(i).getX(), oMinutiae.get().getList().get(i).getY(), oMinutiae.get().getList().get(i).getEx(), oMinutiae.get().getList().get(i).getEy(), ILfs.SCAN_CLOCKWISE, binarizedImageData, imageWidth, imageHeight) == ILfs.FOUND || 
									getContour().searchContour(minutia.getX(), minutia.getY(), lfsParams.getMaxMinutiaDelta(), oMinutiae.get().getList().get(i).getX(), oMinutiae.get().getList().get(i).getY(), oMinutiae.get().getList().get(i).getEx(), oMinutiae.get().getList().get(i).getEy(), ILfs.SCAN_COUNTER_CLOCKWISE, binarizedImageData, imageWidth, imageHeight)  == ILfs.FOUND)
								{
									...
									if (directionMapValue >= ILfs.FALSE)
									{
										...
										mapScanDir = chooseScanDirection(directionMapValue, lfsParams.getNumDirections());
		...
		oMinutiae.get().getList().add(oMinutiae.get().getNum(), minutia);
		//minutiae.get().getList().set(minutiae.get().getNum(), minutia);
		oMinutiae.get().setNum(oMinutiae.get().getNum() + 1);
	...
	/*************************************************************************
	**************************************************************************
	#cat: sortMinutiaeTopToBottomAndThenLeftToRight - Takes a list of minutia points and sorts them
	#cat:                 top-to-bottom and then left-to-right.
	   Input:
	      oMinutiae  - list of minutiae
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	   Output:
	      oMinutiae  - list of sorted minutiae
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int sortMinutiaeTopToBottomAndThenLeftToRight(AtomicReference<Minutiae> oMinutiae, 
		...
		{
			ranks.set(i, (oMinutiae.get().getList().get(i).getY() * imageWidth) + oMinutiae.get().getList().get(i).getX());
		}

		/* Get sorted order of minutiae. */
		if ((ret = getSort().sortIndicesIntArrayIncremental(order, ranks, oMinutiae.get().getNum())) 
			!= ILfs.FALSE)
		{
			getFree().free(ranks);
			return (ret);
		...
		for (i = 0; i < oMinutiae.get().getNum(); i++)
		{
			//newlist.set(i, minutiae.get().getList().get(order.get(i)));
	...
	/*************************************************************************
	**************************************************************************
	#cat: sortMinutiaeLeftToRightAndThenTopToBottom - Takes a list of minutia points and sorts them
	#cat:                 left-to-right and then top-to-bottom.
	   Input:
	      oMinutiae  - list of oMinutiae
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	   Output:
	      oMinutiae  - list of sorted oMinutiae
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int sortMinutiaeLeftToRightAndThenTopToBottom(AtomicReference<Minutiae> oMinutiae, final int imageWidth, final int imageHeight) {
		...
		for (i = 0; i < oMinutiae.get().getNum(); i++)
		{
			ranks.set(i, (oMinutiae.get().getList().get(i).getX() * imageWidth) + oMinutiae.get().getList().get(i).getY());
		}
	...
	/*************************************************************************
	**************************************************************************
	#cat: removeRedundantMinutiaeremoveRedundantMinutiae - Takes a list of minutiae sorted in some adjacent order
	...
	#cat:               attributes may differ).
	   Input:
	      oMinutiae - list of sorted minutiae
	   Output:
	      oMinutiae - list of sorted minutiae with duplicates removed
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int removeRedundantMinutiae(AtomicReference<Minutiae> oMinutiae) {
		...
		for (i = oMinutiae.get().getNum() - 1; i > 0; i--)
		{
			...
			if ((firstMinutia.getX() == secondMinutia.getX()) && 
				(firstMinutia.getY() == secondMinutia.getY()))
			{
	...
	/*************************************************************************
	**************************************************************************
	#cat: dump_minutiae - Given a oMinutiae list, writes a formatted text report of
	#cat:            the list's contents to the specified open file pointer.
	   Input:
	      oMinutiae - list of minutia structures
	   Output:
	...
	**************************************************************************/
	public void dumpMinutiae(File file, AtomicReference<Minutiae> oMinutiae) {
		try {
		    ...
		    myWriter.write(String.format( "\n%d Minutiae Detected\n\n", oMinutiae.get().getNum()));
			int i, j;
			for (i = 0; i < oMinutiae.get().getNum(); i++)
			{
				/* Precision of reliablity added one decimal position */
				/* on 09-13-04 */
				myWriter.write(String.format( "%4d : %4d, %4d : %2d : %6.3f :", i, oMinutiae.get().getList().get(i).getX(), oMinutiae.get().getList().get(i).getY(), oMinutiae.get().getList().get(i).getDirection(), oMinutiae.get().getList().get(i).getReliability()));
				if (oMinutiae.get().getList().get(i).getType() == ILfs.RIDGE_ENDING)
				{
					myWriter.write(String.format( "RIG : "));
				}
				else
				{
					myWriter.write(String.format( "BIF : "));
				}

				if (oMinutiae.get().getList().get(i).getAppearing() == ILfs.APPEARING)
				{
					myWriter.write(String.format( "APP : "));
				}
				else
				{
					myWriter.write(String.format( "DIS : "));
				}

				myWriter.write(String.format("%2d ", oMinutiae.get().getList().get(i).getFeatureId()));

				for (j = 0; j < oMinutiae.get().getList().get(i).getNumNbrs(); j++)
				{
					myWriter.write(String.format(": %4d,%4d; %2d ", oMinutiae.get().getList().get(oMinutiae.get().getList().get(i).getNbrs().get(j)).getX(), 
	...
	/*************************************************************************
	**************************************************************************
	#cat: dumpMinutiaePoints - Given a oMinutiae list, writes the coordinate point
	...
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: dumpReliableMinutiaePoints - Given a oMinutiae list, writes the
	#cat:            coordinate point for each oMinutiae in the list that has
	#cat:            the specified reliability to the specified open
	#cat:            file pointer.
	   Input:
	      oMinutiae    - list of minutia structures
	      reliability - desired reliability level for oMinutiae to be reported
	   Output:
	      file - open file pointer
	**************************************************************************/
	...
	**************************************************************************
	...
	#cat:            point and allocates and initializes a minutia structure.
	   Input:
	      xLoc   - x-pixel coord of minutia (interior to feature)
	      yLoc   - y-pixel coord of minutia (interior to feature)
	      xEdge  - x-pixel coord of corresponding edge pixel (exterior to feature)
	      yEdge  - y-pixel coord of corresponding edge pixel (exterior to feature)
	      ...
	      featureId - index of minutia's matching feature_patterns[]
	   Output:
	      Minutia - ponter to an allocated and initialized minutia structure
	*************************************************************************/
	public Minutia createMinutia(final int xLoc, final int yLoc, 
		final int xEdge, final int yEdge, final int iDir, final double reliability, 
	...
	**************************************************************************
	...
	#cat:                  list of minutiae.
	   Input:
	      ...
	      oMinutiae   - input list of minutiae
	   Output:
	      oMinutiae   - list with minutia removed
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	...
	**************************************************************************
	...
	#cat:                from the interior line.
	   ...
	   Output:
	      binarizedImageData      - edited image with minutia features joined
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	public int joinMinutia(Minutia minutia1, Minutia minutia2, int[] binarizedImageData, 
		...
		int dxGreaterThandy, deltaX, deltaY;
		int[] xList = null, yList = null;
		AtomicInteger oNum = new AtomicInteger(0);
		int minutiaPixel = 0, boundaryPixel;
		int i, j, ret;
		int x1, y1, x2, y2;
		...
		int asize = Math.max(Math.abs(minutia2.getX() - minutia1.getX()) + 2, Math.abs(minutia2.getY() - minutia1.getY()) + 2);
		xList = new int[asize]; 
		yList = new int[asize]; 
		
		if ((ret = getLine().linePoints(xList, yList, oNum, minutia1.getX(), minutia1.getY(), 
			minutia2.getX(), minutia2.getY())) != ILfs.FALSE)
		{
	...
	**************************************************************************
	...
	#cat:            bifurcation (white pixel).
	   Input:
	      featurePixel - pixel color of the feature's interior
	   Return Code:
	...
	**************************************************************************
	...
	#cat:            "feature" refers to either a ridge or valley-ending.
	   Input:
	      xLoc      - x-pixel coord of feature (interior to feature)
	      yLoc      - y-pixel coord of feature (interior to feature)
	      xEdge     - x-pixel coord of corresponding edge pixel
	                   (exterior to feature)
	      yEdge     - y-pixel coord of corresponding edge pixel
	                   (exterior to feature)
	   Return Code:
	      ...
	      Negative     - system error
	**************************************************************************/
	public int isMinutiaAppearing(final int xLoc, final int yLoc,
	...
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: scanForMinutiae - Scans a block of binary image data detecting potential
	#cat:                minutiae points.
	   Input:
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      oInputBlockImageMap      - matrix of ridge flow directions
	      oNMap      - IMAP augmented with blocks of HIGH-CURVATURE and
	                  blocks which have no neighboring valid directions.
	      blockX     - x-block coord to be scanned
	      blockY     - y-block coord to be scanned
	      mapWidth        - width (in blocks) of IMAP and NMAP matrices.
	      mapHeight        - height (in blocks) of IMAP and NMAP matrices.
	      scanX    - x-pixel coord of origin of region to be scanned
	      scanY    - y-pixel coord of origin of region to be scanned
	      scanWidth    - width (in pixels) of region to be scanned
	      scanHeight    - height (in pixels) of region to be scanned
	      scanDir  - the scan orientation (horizontal or vertical)
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oMinutiae   - points to a list of detected minutia structures
	   Return Code:
	      ...
	      Negative  - system error
	**************************************************************************/
	public int scanForMinutiae(AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		final int imageWidth, final int imageHeight, AtomicIntegerArray oInputBlockImageMap, AtomicIntegerArray oNMap, 
		final int blockX, final int blockY, final int mapWidth, final int mapHeight, 
		final int scanX, final int scanY, final int scanWidth, final int scanHeight, 
		final int scanDir, final LfsParams lfsParams) {
		int blockIndex, ret;
		...
		if (scanDir == ILfs.SCAN_HORIZONTAL)
		{
			if ((ret = scanForMinutiaeHorizontally(oMinutiae, binarizedImageData, imageWidth, imageHeight, 
				oInputBlockImageMap.get(blockIndex), oNMap.get(blockIndex), scanX, scanY, 
				scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				/* Return code may be:                      */
			...
			if ((ret = rescanForMinutiaeVertically(oMinutiae, binarizedImageData, imageWidth, imageHeight, 
				oInputBlockImageMap, oNMap, blockX, blockY, mapWidth, mapHeight, scanX, scanY, 
				scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				/* Return code may be:                      */
				/* 1. ret<0 (implying system error)         */
	...
	**************************************************************************
	...
	#cat:                HOWEVER, some minutiae will still be missed!
	   Input:
	      ...
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      nInputBlockImageMapValue   - IMAP value associated with this image region
	      nNMapValue   - NMAP value associated with this image region
	      scanX    - x-pixel coord of origin of region to be scanned
	      scanY    - y-pixel coord of origin of region to be scanned
	      scanWidth    - width (in pixels) of region to be scanned
	      scanHeight    - height (in pixels) of region to be scanned
	...
	**************************************************************************/
	public int scanForMinutiaeHorizontally(AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		...
		int sx, sy, ex, ey;
		AtomicInteger cx = new AtomicInteger(0), cy = new AtomicInteger(0);
		int x2;
		AtomicInteger p1ptrIndex = new AtomicInteger(0), p2ptrIndex = new AtomicInteger(0);
					...
					binarizedImageData[p2ptrIndex.get()], possible, nposs) != ILfs.FALSE)
				{
					...
					if (cx.get() < ex)
					{
							...
							binarizedImageData[p2ptrIndex.get()], possible, nposs) != ILfs.FALSE)
						{
							...
							getMatchPattern().skipRepeatedHorizontalPair(cx, ex, 
								binarizedImageData, p1ptrIndex, p2ptrIndex, imageWidth, imageHeight);
							/* If not at end of region's current scan row... */
							if (cx.get() < ex)
							{
									...
									binarizedImageData[p2ptrIndex.get()], possible, nposs) != ILfs.FALSE)
								{
									...
									if ((ret = processHorizontalScanMinutia(oMinutiae, cx.get(), cy.get(), 
										x2, possible.get(0), binarizedImageData, imageWidth, imageHeight, 
	...
	**************************************************************************
	...
	#cat:                by nature vertically oriented (orthogonal to the scan).
	   Input:
	      ...
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      oDirectionMap  - pixelized Direction Map
	      oLowFlowMap   - pixelized Low Ridge Flow Map
	      oHighCurveMap - pixelized High Curvature Map
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	   ...
	   Return Code:
	      ...
	      Negative  - system error
	**************************************************************************/
	public int scanForMinutiaeHorizontallyV2(AtomicReference<Minutiae> oMinutiae, 
					...
					if (cx.get() < ex)
					{
							...
							binarizedImageData[p2ptrIndex.get()], oPossible, oNoOfPoss) != ILfs.FALSE)
						{
							...
							if (cx.get() < ex)
							{
								...
								{
									/* Process detected minutia point. */
									if ((ret = processHorizontalScanMinutiaV2(oMinutiae, cx.get(), cy.get(), 
										x2, oPossible.get(0), binarizedImageData, imageWidth, imageHeight, 
					...
					binarizedImageData[p2ptrIndex.get()], possible, nposs) != ILfs.FALSE)
				{
					...
					if (cy.get() < ey)
					{
							...
							binarizedImageData[p2ptrIndex.get()], possible, nposs) != ILfs.FALSE)
						{
							...
							y2 = cy.get();
							/* Skip repeated pixel pairs. */
							getMatchPattern().skipRepeatedVerticalPair(cy, ey, binarizedImageData, 
								p1ptrIndex, p2ptrIndex, imageWidth, imageHeight);
							/* If not at end of region's current scan column... */
							if (cy.get() < ey)
							{
								/* If scan pixel pair matches third pixel pair of */
	...
	**************************************************************************
	...
	#cat:                block's neighboring blocks' IMAP and NMAP values.
	   Input:
	      ...
	      oNMap      - IMAP augmented with blocks of HIGH-CURVATURE and
	                  blocks which have no neighboring valid directions.
	      blockX     - x-block coord to be rescanned
	      blockY     - y-block coord to be rescanned
	      mapWidth        - width (in blocks) of IMAP and NMAP matrices.
	      mapHeight        - height (in blocks) of IMAP and NMAP matrices.
	      scanX    - x-pixel coord of origin of region to be rescanned
	      scanY    - y-pixel coord of origin of region to be rescanned
	      scanWidth    - width (in pixels) of region to be rescanned
	      scanHeight    - height (in pixels) of region to be rescanned
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oMinutiae   - points to a list of detected minutia structures
	   Return Code:
	...
	**************************************************************************/
	public int rescanForMinutiaeHorizontally(AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		...
		if (oNMap.get(blockIndex)  == ILfs.HIGH_CURVATURE)
		{
			...
			if ((ret = scanForMinutiaeHorizontally(oMinutiae, binarizedImageData, imageWidth, imageHeight, 
				oInputBlockImageMap.get(blockIndex), oNMap.get(blockIndex), 
				scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				/* Return code may be:                      */
				/* 1. ret<0 (implying system error)          */
				return (ret);
		...
		else
		{
			...
			if ((ret = rescanPartialHorizontally(ILfs.NORTH, oMinutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				/* Return code may be:                      */
				/* 1. ret<0 (implying system error)          */
				return (ret);
			...
			if ((ret = rescanPartialHorizontally(ILfs.EAST, oMinutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				return (ret);
			...
			if ((ret = rescanPartialHorizontally(ILfs.SOUTH, oMinutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				return (ret);
			...
			if ((ret = rescanPartialHorizontally(ILfs.WEST, oMinutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				return (ret);
	...
	/*************************************************************************
	**************************************************************************
	#cat: scanForMinutiaeVerticallyV2 - Scans an entire binary image
	#cat:                vertically, detecting potential minutiae points.
	#cat:                Minutia detected via the vetical scan process are
	#cat:                by nature horizontally oriented (orthogonal to  the scan).
	   Input:
	      ...
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      Maps		-- contains all below information
	      oDirectionMap  - pixelized Direction Map
	      oLowFlowMap   - pixelized Low Ridge Flow Map
	      oHighCurveMap - pixelized High Curvature Map
	      lfsParams  - parameters and thresholds for controlling LFS
	...
	**************************************************************************/
	public int scanForMinutiaeVerticallyV2(AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		final int imageWidth, final int imageHeight, AtomicIntegerArray oDirectionMap, AtomicIntegerArray oLowFlowMap, 
		AtomicIntegerArray oHighCurveMap, final LfsParams lfsParams) {
		int sx, sy, ex, ey;
		AtomicInteger cx = new AtomicInteger(0), cy = new AtomicInteger(0);
		AtomicInteger p1ptrIndex = new AtomicInteger(0), p2ptrIndex = new AtomicInteger(0);
					...
					if (cy.get() < ey)
					{
							...
							binarizedImageData[p2ptrIndex.get()], pPossible, oPoss) != ILfs.FALSE)
						{
							...
							if (cy.get() < ey)
							{
								...
								{
									/* Process detected minutia point. */
									if ((ret = processVerticalScanMinutiaV2(oMinutiae, cx.get(), 
										cy.get(), y2, pPossible.get(0), binarizedImageData, imageWidth, imageHeight, 
										oDirectionMap, oLowFlowMap, oHighCurveMap, lfsParams)) != ILfs.FALSE)
									{
										/* Return code may be:                       */
										/* 1.  ret< 0 (implying system error)        */
	...
	**************************************************************************/
	public int rescanForMinutiaeVertically(AtomicReference<Minutiae> minutiae, int[] binarizedImageData, 
		...
		else
		{
			...
			if ((ret = rescanPartialVertically(ILfs.NORTH, minutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				/* Return code may be:                      */
			...
			if ((ret = rescanPartialVertically(ILfs.EAST, minutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				return (ret);
			...
			if ((ret = rescanPartialVertically(ILfs.SOUTH, minutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				return (ret);
			...
			if ((ret = rescanPartialVertically(ILfs.WEST, minutiae, 
				binarizedImageData, imageWidth, imageHeight, oInputBlockImageMap, oNMap, 
				blockX, blockY, mapWidth, mapHeight, scanX, scanY, scanWidth, scanHeight, lfsParams)) != ILfs.FALSE)
			{
				return (ret);
	...
	**************************************************************************
	...
	#cat:                of a specified neighboring block.
	   Input:
	      nbrDir   - specifies which block neighbor {NORTH, SOUTH, EAST, WEST}
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      oInputBlockImageMap      - matrix of ridge flow directions
	      ...
	      mapWidth        - width (in blocks) of IMAP and NMAP matrices.
	      mapHeight        - height (in blocks) of IMAP and NMAP matrices.
	      scanX    - x-pixel coord of origin of image region
	      scanY    - y-pixel coord of origin of image region
	      scanWidth    - width (in pixels) of image region
	      scanHeight    - height (in pixels) of image region
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oMinutiae   - points to a list of detected minutia structures
	   Return Code:
	...
	**************************************************************************/
	public int rescanPartialHorizontally(final int nbrDir, AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		...
		int blockIndex;
		int rescanDir;
		AtomicInteger rescanX = new AtomicInteger(0), rescanY = new AtomicInteger(0), 
			rescanWidth = new AtomicInteger(0), rescanHeight = new AtomicInteger(0);
		int ret;
		...
		if (oInputBlockImageMap.get(oBlockIndex.get()) != ILfs.INVALID_DIR)
		{
			...
			if (rescanDir == ILfs.SCAN_HORIZONTAL)
			{
				...
				if ((ret = adjustHorizontalRescan(nbrDir, rescanX, rescanY, rescanWidth, rescanHeight, 
					scanX, scanY, scanWidth, scanHeight, lfsParams.getBlockSize())) != ILfs.FALSE)
				{
	...
	/*************************************************************************
	**************************************************************************
	#cat: rescanPartialVertically - Rescans a portion of a block of binary
	...
	**************************************************************************/
	public int rescanPartialVertically(final int nbrDir, AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		...
		int rescan_dir;
		AtomicInteger rescanX = new AtomicInteger(0), rescanY = new AtomicInteger(0);
		AtomicInteger rescanWidth = new AtomicInteger(0), rescanHeight = new AtomicInteger(0);
		int ret;
		...
		{
			/* Compute block index from current (not neighbor) block coordinates. */
			blk_i = (blockY * mapWidth) + blockX;

			/* Select feature scan direction based on neighbor IMAP. */
			rescan_dir = chooseScanDirection(oInputBlockImageMap.get(oBlockIndex.get()), lfsParams.getNumDirections());
			/* If new scan direction is VERTICAL... */
			if (rescan_dir == ILfs.SCAN_VERTICAL)
			{
				/* Adjust scanX, scanY, scanWidth, scanHeight for rescan. */
				if ((ret = adjustVerticalRescan(nbrDir, rescanX, rescanY, rescanWidth, rescanHeight, 
					scanX, scanY, scanWidth, scanHeight, lfsParams.getBlockSize())) != ILfs.FALSE)
				{
					/* Return system error code. */
					return (ret);
				...
				/* Pass IMAP direction for the block, NOT its neighbor. */
				if ((ret = scanForMinutiaeVertically(oMinutiae, binarizedImageData, imageWidth, imageHeight, 
					oInputBlockImageMap.get(blk_i), oNMap.get(blk_i), rescanX.get(), rescanY.get(), 
					rescanWidth.get(), rescanHeight.get(), lfsParams)) != ILfs.FALSE)
				{
					/* Return code may be:                      */
					/* 1. ret<0 (implying system error)          */
	...
	/*************************************************************************
	**************************************************************************
	#cat: getNbrBlockIndex - Determines the block index (if one exists)
	#cat:                for a specified neighbor of a block in the image.
	   Input:
	      nbrDir   - specifies which block neighbor {NORTH, SOUTH, EAST, WEST}
	      blockX     - x-block coord to find neighbor of
	      blockY     - y-block coord to find neighbor of
	      mapWidth        - width (in blocks) of IMAP and NMAP matrices.
	      mapHeight        - height (in blocks) of IMAP and NMAP matrices.
	   Output:
	      oBlockIndex    - points to neighbor's block index
	   Return Code:
	      NOT_FOUND - neighbor index does not exist
	      FOUND     - neighbor index exists and returned
	      Negative  - system error
	**************************************************************************/
	public int getNbrBlockIndex(AtomicInteger oBlockIndex, final int nbrDir, final int blockX, 
		final int blockY, final int mapWidth, final int mapHeight) {
		int nx, ny, ni;
	...
	**************************************************************************
	...
	#cat:                be rescanned horizontally based on a specified neighbor.
	   ...
	   Output:
	      rescanX    - x-pixel coord of origin of region to be rescanned
	      rescanY    - y-pixel coord of origin of region to be rescanned
	      rescanWidth    - width (in pixels) of region to be rescanned
	      rescanHeight    - height (in pixels) of region to be rescanned
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	public int adjustHorizontalRescan(final int nbrDir, AtomicInteger rescanX, AtomicInteger rescanY, 
		...
		int halfBlocksize, qtrBlocksize;
	...
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: processHorizontalScanMinutia - Takes a minutia point that was
	...
	#cat:                vertical in orientation (orthogonal to the scan).
	   Input:
	      ...
	      featureId - type of minutia (ex. index into feature_patterns[] list)
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      nInputBlockImageMapValue   - IMAP value associated with this image region
	...
	**************************************************************************/
	public int processHorizontalScanMinutia(AtomicReference<Minutiae> oMinutiae, final int cx, final int cy, 
		...
		Minutia minutia;
		AtomicInteger xLoc = new AtomicInteger(0), yLoc = new AtomicInteger(0), 
			xEdge = new AtomicInteger(0), yEdge = new AtomicInteger(0), iDir = new AtomicInteger(0);
		int ret;
		...
		if (getGlobals().getFeaturePatterns()[featureId].appearing >= ILfs.APPEARING)
		{
		...
		if (nNMapValue == ILfs.HIGH_CURVATURE)
		{
			...
			if ((ret = adjustHighCurvatureMinutia(iDir, xLoc, yLoc, xEdge, yEdge, 
				xLoc.get(), yLoc.get(), xEdge.get(), yEdge.get(), 
				binarizedImageData, imageWidth, imageHeight, oMinutiae, lfsParams)) != ILfs.FALSE)
			{
		...
		else
		{
			...
			iDir.set(getLowCurvatureDirection(ILfs.SCAN_HORIZONTAL, 
				getGlobals().getFeaturePatterns()[featureId].appearing, nInputBlockImageMapValue, 
				lfsParams.getNumDirections()));
		}

		/* Create a minutia object based on derived attributes. */
		minutia = createMinutia(xLoc.get(), yLoc.get(), xEdge.get(), yEdge.get(), iDir.get(), 
			ILfs.DEFAULT_RELIABILITY, getGlobals().getFeaturePatterns()[featureId].type, 
			getGlobals().getFeaturePatterns()[featureId].appearing, featureId);
	...
	/*************************************************************************
	**************************************************************************
	#cat: processHorizontalScanMinutiaV2 - Takes a minutia point that was
	#cat:                detected via the horizontal scan process and
	#cat:                adjusts its location (if necessary), determines its
	#cat:                direction, and (if it is not already in the minutiae
	#cat:                list) adds it to the list.  These minutia are by nature
	...
	**************************************************************************/
	public int processHorizontalScanMinutiaV2(AtomicReference<Minutiae> oMinutiae, final int cx, final int cy, 
		final int x2, final int featureId, int[] binarizedImageData, final int imageWidth, final int imageHeight,
		AtomicIntegerArray oDirectionMap, AtomicIntegerArray oLowFlowMap, 
		AtomicIntegerArray oHighCurveMap, final LfsParams lfsParams) {
		Minutia minutia = new Minutia();
		AtomicInteger xLoc = new AtomicInteger(0), yLoc = new AtomicInteger(0), 
			xEdge = new AtomicInteger(0), yEdge = new AtomicInteger(0);
		AtomicInteger iDir = new AtomicInteger(0);

		int ret;
		int directionMapValue, lowFlowMapValue, highCurveMapValue;
		double reliability;
		...
		{
			/* Adjust location and direction locally. */
			ret = adjustHighCurvatureMinutiaV2(iDir, xLoc, yLoc, xEdge, yEdge, xLoc.get(), yLoc.get(), 
				xEdge.get(), yEdge.get(), binarizedImageData, imageWidth, imageHeight, 
				oLowFlowMap, oMinutiae, lfsParams);
			if (ret != ILfs.FALSE)
			{
				/* Could be a system error or IGNORE minutia. */
				return (ret);
		...
		else
		{
			...
			iDir.set(getLowCurvatureDirection(ILfs.SCAN_HORIZONTAL, 
					getGlobals().getFeaturePatterns() [featureId].appearing, directionMapValue, 
					lfsParams.getNumDirections()));
		}
		...
		/* Create a minutia object based on derived attributes. */
		minutia = createMinutia(xLoc.get(), yLoc.get(), xEdge.get(), yEdge.get(), iDir.get(), 
			reliability, getGlobals().getFeaturePatterns()[featureId].type, 
			getGlobals().getFeaturePatterns()[featureId].appearing, featureId);
		
		/* Update the minutiae list with potential new minutia. */
		ret = updateMinutiaeV2(oMinutiae, minutia, ILfs.SCAN_HORIZONTAL, directionMapValue, 
	...
	/*************************************************************************
	**************************************************************************
	#cat: processVerticalScanMinutia - Takes a minutia point that was
	...
	**************************************************************************/
	public int processVerticalScanMinutia(AtomicReference<Minutiae> oMinutiae, 
		...
		final int imageWidth, final int imageHeight, final int nInputBlockImageMapValue, 
		final int nNMapValue, final LfsParams lfsParams) {
		Minutia minutia;
		AtomicInteger xLoc = new AtomicInteger(0), yLoc = new AtomicInteger(0);
		AtomicInteger xEdge = new AtomicInteger(0), yEdge = new AtomicInteger(0);
		AtomicInteger iDir = new AtomicInteger(0);
		int ret;
		...
		{
			/* Get minutia direction based on current IMAP value. */
			iDir.set(getLowCurvatureDirection(ILfs.SCAN_VERTICAL, getGlobals().
				getFeaturePatterns() [featureId].appearing, nInputBlockImageMapValue, lfsParams.getNumDirections()));
		}
	...
	/*************************************************************************
	**************************************************************************
	#cat: processVerticalScanMinutiaV2 - Takes a minutia point that was
	#cat:                detected in via the vertical scan process and
	#cat:                adjusts its location (if necessary), determines its
	#cat:                direction, and (if it is not already in the minutiae
	#cat:                list) adds it to the list.  These minutia are by nature
	...
	**************************************************************************/
	public int processVerticalScanMinutiaV2(AtomicReference<Minutiae> oMinutiae, 
		...
		{
			/* Adjust location and direction locally. */
			ret = adjustHighCurvatureMinutiaV2(iDir, xLoc, yLoc, xEdge, yEdge, 
				xLoc.get(), yLoc.get(), xEdge.get(), yEdge.get(), 
				binarizedImageData, imageWidth, imageHeight, oLowFlowMap, oMinutiae, lfsParams);
			if (ret != ILfs.FALSE)
			{
				/* Could be a system error or IGNORE minutia. */
				return (ret);
		...
		else
		{
			...
			iDir.set(getLowCurvatureDirection(ILfs.SCAN_VERTICAL, 
				getGlobals().getFeaturePatterns() [featureId].appearing, 
				directionMapValue, lfsParams.getNumDirections()));
		}
		...
		/* Create a minutia object based on derived attributes. */
		minutia = createMinutia(xLoc.get(), yLoc.get(), xEdge.get(), yEdge.get(), 
			iDir.get(), reliability, getGlobals().getFeaturePatterns() [featureId].type, 
			getGlobals().getFeaturePatterns() [featureId].appearing, featureId);

		/* Update the minutiae list with potential new minutia. */
		ret = updateMinutiaeV2(oMinutiae, minutia, ILfs.SCAN_VERTICAL, directionMapValue, 
				binarizedImageData, imageWidth, imageHeight, lfsParams);
		/* If minuitia IGNORED and not added to the minutia list ... */
		if (ret == ILfs.IGNORE)
		{
	...
	**************************************************************************
	...
	#cat:            direction.
	   Input:
	      xLoc     - starting x-pixel coord of feature (interior to feature)
	      yLoc     - starting y-pixel coord of feature (interior to feature)
	      xEdge    - x-pixel coord of corresponding edge pixel
	                  (exterior to feature)
	      yEdge    - y-pixel coord of corresponding edge pixel
	                  (exterior to feature)
	   ...
	   Output:
	      ...
	      oXLoc    - adjusted x-pixel coord of feature
	      oYLoc    - adjusted y-pixel coord of feature
	      oXEdge   - adjusted x-pixel coord of corresponding edge pixel
	      oYEdge   - adjusted y-pixel coord of corresponding edge pixel
	      oMinutiae   - points to a list of detected minutia structures
	   Return Code:
	...
	**************************************************************************/
	public int adjustHighCurvatureMinutia(AtomicInteger oIDir, AtomicInteger oXLoc, AtomicInteger oYLoc, 
		AtomicInteger oXEdge, AtomicInteger oYEdge, final int xLoc, final int yLoc, 
		final int xEdge, final int yEdge, int[] binarizedImageData, final int imageWidth, final int imageHeight, 
		AtomicReference<Minutiae> oMinutiae, final LfsParams lfsParams) {
		Contour contour = null;
		AtomicInteger ret = new AtomicInteger(0);
		AtomicInteger oNoOfContour = new AtomicInteger(0), oMinIndex = new AtomicInteger(0);
		AtomicReference<Double> oMinTheta = new AtomicReference<Double>(0.0);
		int midX, midY, midPixel, featurePixel;
		int iDir;
		int halfContour;
		int angleEdge;
		...
		contour = getContour().getHighCurvatureContour(ret, oNoOfContour, halfContour, 
			xLoc, yLoc, xEdge, yEdge, binarizedImageData, imageWidth, imageHeight);
		if (ret.get() != ILfs.FALSE)
		{
			/* Returns with:                                                    */
			...
			if (ret.get() == ILfs.LOOP_FOUND)
			{
				...
				ret.set(getLoop().isLoopClockwise(contour.getContourX(), contour.getContourY(), 
					oNoOfContour.get(), ILfs.TRUE));
				if (ret.get() != ILfs.FALSE)
				{
				...
				ret.set(getLoop().processLoop(oMinutiae, contour.getContourX(), contour.getContourY(), 
					contour.getContourEx(), contour.getContourEy(), oNoOfContour.get(), 
					binarizedImageData, imageWidth, imageHeight, lfsParams));
				/* Returns with:                              */
		...
		ret.set(getContour().minContourTheta(oMinIndex, oMinTheta, angleEdge, 
			contour.getContourX(), contour.getContourY(), oNoOfContour.get()));
		if (ret.get() != ILfs.FALSE)
		{
			/* Deallocate contour lists. */
			getContour().freeContour(contour);
		...
		midX = (contour.getContourX().get(oMinIndex.get() - angleEdge) + contour.getContourX().get(oMinIndex.get() + angleEdge)) >> 1;
		midY = (contour.getContourY().get(oMinIndex.get() - angleEdge) + contour.getContourY().get(oMinIndex.get() + angleEdge)) >> 1;
	...
	/*************************************************************************
	**************************************************************************
	#cat: adjustHighCurvatureMinutiaV2 - Takes an initial minutia point
	#cat:            in a high-curvature area and adjusts its location and
	#cat:            direction.  First, it walks and extracts the contour
	#cat:            of the detected feature looking for and processing any loop
	#cat:            discovered along the way.  Once the contour is extracted,
	...
	**************************************************************************/
	public int adjustHighCurvatureMinutiaV2(AtomicInteger oIDir, AtomicInteger oXLoc, AtomicInteger oYLoc, 
		...
		AtomicReference<Double> oMinTheta = new AtomicReference<Double>(0.0d);
		int featurePixel;
		int midX, midY, midPixel;
		int iDir;
		int halfContour;
		int angleEdge;
		...
		if (ret.get() != ILfs.FALSE)
		{
			...
			if (ret.get() == ILfs.LOOP_FOUND)
			{
				...
				ret.set(getLoop().processLoopV2(oMinutiae, contour.getContourX(), contour.getContourY(), 
						contour.getContourEx(), contour.getContourEy(), oNoOfContour.get(), 
						binarizedImageData, imageWidth, imageHeight, oLowFlowMap, lfsParams));
				/* Returns with:                              */
				/*    1. Successful processing of loop == 0   */
				/*    2. System error < 0                     */
	...
	**************************************************************************
	...
	#cat:            detected minutia is appearing or disappearing.
	   ...
	   Input:
	      scanDir   - designates the feature scan orientation
	      appearing  - designates the minutia as appearing or disappearing
	      nInputBlockImageMapValue    - IMAP block direction
	...
	**************************************************************************
	...
	#cat:                 associated with it.
	   Input:
	      oMinutiae - pointer to allocated list of minutia structures
	*************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: freeMinutia - Takes a minutia pointer and deallocates all memory
	#cat:            associated with it.
	   Input:
