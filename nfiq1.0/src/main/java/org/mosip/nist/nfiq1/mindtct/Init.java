...
public class Init extends MindTct implements IInit {
	...
	**************************************************************************
	...
	#cat:                   need to repad for further processes.
	   Input:
	      iMapBlockSize  - the size (in pixels) of each IMAP block in the image
	      dirBinGridWidth   - the width (in pixels) of the rotated grids used in
	                        directional binarization
	      dirBinGridHeight   - the height (in pixels) of the rotated grids used in
	                        directional binarization
	...
	**************************************************************************/
	public int getMaxPadding(final int iMapBlockSize, final int dirBinGridWidth, 
		final int dirBinGridHeight, final int isobin_grid_dim) {
		int dftPad, dirBinPad, isoBinPad, maxPad;
		double diag, pad;

		/* Compute pad required for rotated blocks used in DFT analyses. */
		diag = Math.sqrt((double)(2.0 * iMapBlockSize * iMapBlockSize));
		...
		pad = (diag - iMapBlockSize) / (double)2.0;
	...
	**************************************************************************
	...
	#cat:         the input image avoiding the need to repad for further processes.
	   Input:
	      mapWindowSize  - the size (in pixels) of each window centered about
	                        each block in the image used in DFT analyses
	      mapWindowOffset - the offset (in pixels) from the orgin of the
	   ...
	   Return Code:
	      Non-negative - the maximum padding required for all processes
	**************************************************************************/
	public int getMaxPaddingV2(final int mapWindowSize, final int mapWindowOffset, 
		final int dirBinGridWidth, final int dirBinGridHeight) {
		int dftPad, dirBinPad, maxPad;
		double diag, pad;
		...
		/* Explanation of DFT padding:
		         ...
		         Distance C = Number of pixels in which the window extends
		                   beyond the image (mapWindowOffset).
		...
		*/
		...
		/* (DFT grids are computed with pixel offsets RELATIVE2ORIGIN.)   */
		diag = Math.sqrt((double)(2.0 * mapWindowSize * mapWindowSize));
		pad = (diag - mapWindowSize) / (double)2.0;
		/* Need to truncate precision so that answers are consistent  */
		/* on different computer architectures when rounding doubles. */
		pad = getDefs().truncDoublePrecision(pad, ILfs.TRUNC_SCALE);
	...
	**************************************************************************
	...
	#cat:                directions to angles in radians.
	   Input:
	      DirToRad - DirToRad Object
          // create at the object creation constructor 
      	  //points to the allocated/initialized DIR2RAD structure
	   Output:
	      ret  - values below
	   Return Code:
	...
	**************************************************************************
	...
	#cat:                 to conduct DFT analysis on blocks of the input image
	   Input:
	      DftWaves - DftWaves Object
          // create at the object creation constructor 
      	  // points to the allocated/initialized DFTWAVES structure
	      dftCoefs - array of multipliers used to define the frequency for
	...
	**************************************************************************/
	public int initDftWaves(DftWaves dftWaves, AtomicReferenceArray<Double> dftCoefs) {
		...
		piFactor = 2.0 * Math.PI / (double)dftWaves.getWaveLen();
	...
	**************************************************************************
	...
	#cat:                 in isotropic binarization.
	   Input:
	      RotGrids - RotGrids Object
          // create at the object creation constructor 
      	  // points to the allcated/initialized ROTGRIDS structure
	...
	**************************************************************************/
	public int initRotGrids(RotGrids rotGrids, final int imageWidth, final int imageHeight, final int nPad) {
		double piOffset, piIncrement;
		int dir, ix, iy, //gridSize, 
			pw, gridPad, minDim;
		double diag, theta, cos, sin, cx, cy;
		double fxm, fym, fx, fy;
		int ixt, iyt;
		double pad;

		/* Compute pad based on diagonal of the grid */
		diag = Math.sqrt((double)((rotGrids.getGridWidth() * rotGrids.getGridWidth()) + (rotGrids.getGridHeight() * rotGrids.getGridHeight())));
		switch (rotGrids.getRelative2())
		{
			...
			case ILfs.RELATIVE_TO_ORIGIN:
				...
				break;
			default:
				LOGGER.error("ERROR : init_rotgrids : Illegal relative flag : %d\n" + rotGrids.getRelative2());
				rotGrids = null;
				return (-31);
		}
		...
		//gridSize = rotGrids.getGridWidth() * rotGrids.getGridHeight();

		/* Compute width of "padded" image */
		pw = imageWidth + (rotGrids.getPad() << 1);
		//LOGGER.info(String.format("init_rotgrids...optr.getPad() = %d, gridSize = %d, pw = %d, imageWidth = %d, value = %d\n", optr.getPad(), gridSize, pw, imageWidth, (optr.getPad() << 1)));

		//LOGGER.info(String.format("init_RotGrids>>pw="+ pw + " >>imageWidth= " + imageWidth + " <<<pad>>" + optr.getPad()+ " <<<pad1>>" + (optr.getPad() << 1)));

		/* Center coord of grid (0-oriented). */
		cx = (rotGrids.getGridWidth() - 1) / (double)2.0;
		cy = (rotGrids.getGridHeight() - 1) / (double)2.0;

		//LOGGER.info(String.format("init_RotGrids>>gridSize="+ gridSize + " >>pw= " + pw + " <<<cx>>" + cx+ " <<<cy>>" + cy));
		...
		piIncrement = ILfs.M_PI / (double)rotGrids.getNoOfGrids(); // if nDirs == 16, incr = 11.25 degrees
	...
	/*************************************************************************
	**************************************************************************
	#cat: allocDirPowers - Allocates the memory associated with DFT power
	...
	**************************************************************************/
	public AtomicReferenceArray<Double[]> allocDirPowers(AtomicInteger ret, final int nWaves, final int nDirs) {
		...
		ret.set(ILfs.FALSE);
		return new AtomicReferenceArray<Double[]> (oPowers);
	}
	...
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: allocPowerStatsPowmaxs - Allocates memory associated with set of statistics
	#cat:             derived from DFT power vectors computed in a block of the
	#cat:             input image.  Statistics are not computed for the lowest DFT
	#cat:             wave form, so the length of the statistics arrays is 1 less
	#cat:             than the number of DFT wave forms used.  The staistics
	...
	**************************************************************************/
	public AtomicReferenceArray<Double> allocPowerStatsPowmaxs(AtomicInteger ret, final int nStats) {
		...
		ret.set(ILfs.FALSE);
		return new AtomicReferenceArray<Double> (powmaxs);
	}

	/*************************************************************************
	**************************************************************************
	#cat: allocPowerStatsPowmaxDirs - Allocates memory associated with set of statistics
	#cat:             derived from DFT power vectors computed in a block of the
	#cat:             input image.  Statistics are not computed for the lowest DFT
	#cat:             wave form, so the length of the statistics arrays is 1 less
	#cat:             than the number of DFT wave forms used.  The staistics
	...
	**************************************************************************/
	public AtomicIntegerArray allocPowerStatsPowmaxDirs(AtomicInteger ret, final int nStats) {
		...
		ret.set(ILfs.FALSE);
		return new AtomicIntegerArray (powmax_dirs);
	}

	/*************************************************************************
	**************************************************************************
	#cat: allocPowerStatsPownorms - Allocates memory associated with set of statistics
	#cat:             derived from DFT power vectors computed in a block of the
	#cat:             input image.  Statistics are not computed for the lowest DFT
	#cat:             wave form, so the length of the statistics arrays is 1 less
	#cat:             than the number of DFT wave forms used.  The staistics
	...
	**************************************************************************/
	public AtomicReferenceArray<Double> allocPowerStatsPownorms(AtomicInteger ret, final int nStats) {
		...
		ret.set(ILfs.FALSE);
		return new AtomicReferenceArray<Double> (pownorms);
	}
}
