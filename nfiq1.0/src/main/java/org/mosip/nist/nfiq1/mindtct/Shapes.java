...
public class Shapes extends MindTct implements IShapes {
	...
	private static volatile Shapes instance;
	...
	/*************************************************************************
	**************************************************************************
	#cat: Constructor allocShape - Allocates and initializes a shape structure given the
	#cat:              the X and Y limits of the shape.
	   ...
	   Output:
	      ret		- Zero     - Shape successfully allocated and initialized
	...
	**************************************************************************/
	public Shape allocShape(AtomicInteger ret, int xMin, int yMin, 
		...
		/* the shape exactly.                                              */
		shape.setRows(new AtomicReferenceArray<Rows>(alloc_rows));
		...
		shape.setAlloc(alloc_rows);
		shape.setNRows(alloc_rows);
		...
		for (i = 0, y = yMin; i < alloc_rows; i++, y++)
		{
			...
			shape.getRows().get(i).setXs(new AtomicIntegerArray(alloc_pts));
			
			/* Initialize the current row structure's attributes. */
			shape.getRows().get(i).setY(y);
			shape.getRows().get(i).setAlloc(alloc_pts);
	...
	/*************************************************************************
	**************************************************************************
	#cat: freeShape - Deallocates a shape structure and all its allocated
	#cat:              attributes.
	   Input:
	...
	**************************************************************************
	...
	#cat:            as formatted text to the specified open file pointer.
	   ...
	   Output:
	      file     - open file pointer to be written to
	**************************************************************************/
	public void dumpShape(File file, Shape shape) {
		...
		try {
			...
			myWriter.write(String.format( "shape:  ymin=%d, ymax=%d, nrows=%d\n", shape.getYMin(), shape.getYMax(), shape.getNRows()));

			/* Foreach row in the shape... */
			for (i = 0; i < shape.getNRows(); i++)
			{
				/* Print the current row's y-coord and number of points on the row. */
				myWriter.write(String.format( "row %d :   y=%d, npts=%d\n", i, shape.getRows().get(i).getY(), shape.getRows().get(i).getNoOfPts()));
				/* Print each successive point on the current row. */
				for (j = 0; j < shape.getRows().get(i).getNoOfPts(); j++)
				{
					myWriter.write(String.format( "pt %d : %d %d\n", j, shape.getRows().get(i).getXs().get(j), shape.getRows().get(i).getY()));
				}
			}
	...
	**************************************************************************
	...
	#cat:            are stored in left-to-right order.
	   ...
	   Output:
	      shape     - points to the resulting shape structure
	   Return Code:
	      Zero      - shape successfully derived
	      Negative  - system error
	**************************************************************************/
	public Shape shapeFromContour(AtomicInteger ret, AtomicIntegerArray oContourX, 
		AtomicIntegerArray oContourY, final int noOfContour) {
		Shape shape = null;
		Rows row;
		int i;
		AtomicInteger xmin = new AtomicInteger(0), ymin = new AtomicInteger(0),
			xmax = new AtomicInteger(0), ymax = new AtomicInteger(0);
	...
	/*************************************************************************
	**************************************************************************
	#cat: sortRowLeftToRightOnX - Takes a row structure and sorts its points left-to-
	#cat:            right on X.
	   Input:
