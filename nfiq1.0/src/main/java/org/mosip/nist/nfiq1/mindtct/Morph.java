...
public class Morph extends MindTct implements IMorph {
	private static volatile Morph instance;
    public static Morph getInstance() {
        if (instance == null) {
            synchronized (Morph.class) {
                if (instance == null) {
                    instance = new Morph();
                }
            }
	...
	**************************************************************************
	...
	#cat:             erode pixels indiscriminately along the image border.
	   Input:
	      inputImageData       - input 8-bit image to be eroded
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	   Output:
	      outputImageData       - contains to the resulting eroded image
	**************************************************************************/
		...
		for (row = 0 ; row < imageHeight ; row++)
		{
			for (col = 0 ; col < imageWidth ; col++)
			{
				if ((inputImageData[inputImageDataIndex]) == ILfs.TRUE) // 1 erode only operates on true pixels
				{
					...
					if (!(getWest8_2(inputImageData, inputImageDataIndex, col, 1) == ILfs.TRUE && //1
						getEast8_2(inputImageData, inputImageDataIndex, col, imageWidth, 1) == ILfs.TRUE && //1 
						getNorth8_2(inputImageData, inputImageDataIndex, row, imageWidth, 1)  == ILfs.TRUE && //1 
						getSouth8_2(inputImageData, inputImageDataIndex, row, imageWidth, imageHeight, 1) == ILfs.TRUE)//1
						)
					{
	...
	/*************************************************************************
	**************************************************************************
	#cat: dilateImage2 - Dilates an 8-bit image by setting false pixels to
	...
	#cat:             The input image remains unchanged.
	   Input:
	      inputImageData       - input 8-bit image to be dilated
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	   Output:
	      outputImageData       - contains to the resulting dilated image
	**************************************************************************/
	public void dilateImage2(int[] inputImageData, int[] outputImageData, 
		...
		for (row = 0 ; row < imageHeight ; row++)
		{
			for (col = 0 ; col < imageWidth ; col++)
			{
				if ((inputImageData[inputImageDataIndex]) == ILfs.FALSE) /* pixel is already true, neighbors irrelevant */
				{
					/* more efficient with C's left to right evaluation of     */
					/* conjuctions. E N S functions not executed if W is false */
					if (getWest8_2(inputImageData, inputImageDataIndex, col, 0) == ILfs.TRUE || 
						getEast8_2(inputImageData, inputImageDataIndex, col, imageWidth, 0) == ILfs.TRUE || 
						getNorth8_2(inputImageData, inputImageDataIndex, row, imageWidth, 0)  == ILfs.TRUE || 
						getSouth8_2(inputImageData, inputImageDataIndex, row, imageWidth, imageHeight, 0) == ILfs.TRUE
						)
					{
	...
	**************************************************************************
	...
	#cat:                current pixel if defined else it returns  (char)0.
	   Input:
	      inputImageData       - points to current pixel in image
	      inputImageDataIndex- current pixel index
	...
	**************************************************************************/
	public int getSouth8_2(int[] inputImageData, int inputImageDataIndex, int row, int imageWidth, int imageHeight, int failCode) {
	...
	**************************************************************************
	...
	#cat:                current pixel if defined else it returns  (char)0.
	   ...
	   Return Code:
	      ...
	      Pixel     - otherwise, value of neighboring pixel
	**************************************************************************/
	public int getNorth8_2(int[] inputImageData, int inputImageDataIndex, int row, int imageWidth, int failCode) {
		if (row < 1)     /* catch case where image is undefined northwards   */
			return failCode; /* use plane geometry and return code.           */

		//return *(inputImageData-imageWidth);
		return inputImageData[inputImageDataIndex -imageWidth];
	}
	...
	**************************************************************************
	...
	#cat:               current pixel if defined else it returns  (char)0.
	   ...
	   Return Code:
	      ...
	      Pixel     - otherwise, value of neighboring pixel
	**************************************************************************/
	public int getEast8_2(int[] inputImageData, int inputImageDataIndex, int col, int imageWidth, int failCode) {
	...
	**************************************************************************
	...
	#cat:              current pixel if defined else it returns  (char)0.
	   ...
	   Return Code:
	      ...
	      Pixel     - otherwise, value of neighboring pixel
	**************************************************************************/
	public int getWest8_2(int[] inputImageData, int inputImageDataIndex, int col, int failCode) {
