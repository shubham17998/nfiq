...
public class Ridges extends MindTct implements IRidges {
	...
	private static volatile Ridges instance;
    public static Ridges getInstance() {
        if (instance == null) {
	...
	**************************************************************************
	...
	#cat:                each of its neighbors.
	   ...
	   Output:
	      oMinutiae  - list of oMinutiae augmented with neighbors and ridge counts
	   Return Code:
	...
	**************************************************************************/
	public int countMinutiaeRidges(AtomicReference<Minutiae> oMinutiae, int[] binarizedImageData, 
		...
		if ((ret = getMinutiaHelper().sortMinutiaeLeftToRightAndThenTopToBottom(oMinutiae, imageWidth, imageHeight))!= ILfs.FALSE)
		{
			return (ret);
		}
		...
		for (minutiaIndex = 0; minutiaIndex < oMinutiae.get().getNum() - 1; minutiaIndex++)
		{
			...
			/*       oMinutiae->list[i].                                 */
			if ((ret = countMinutiaRidges(minutiaIndex, oMinutiae, 
	...
	/*************************************************************************
	**************************************************************************
	#cat: countMinutiaRidges - Takes a minutia, and determines its closest
	#cat:                neighbors and counts the number of interveining ridges
	#cat:                between the minutia point and each of its neighbors.
	   Input:
	      oMinutiae   - input minutia
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	      lfsParams  - parameters and thresholds for controlling LFS
	   Output:
	      oMinutiae  - minutia augmented with neighbors and ridge counts
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	public int countMinutiaRidges(final int first, AtomicReference<Minutiae> oMinutiae, 
		int[] binarizedImageData,  final int imageWidth, final int imageHeight, final LfsParams lfsParams) {
		int i, ret;
		AtomicIntegerArray nbrList, nbr_nridges;
		...
		if (isShowLogs())
			LOGGER.info(String.format("NBRS FOUND: %d, %d = %d\n", oMinutiae.get().getList().get(first).getX(), oMinutiae.get().getList().get(first).getY(), oNoOfNbrs.get()));
		...
		if ((ret = sortNeighbors(nbrList, oNoOfNbrs.get(), first, oMinutiae)) 
			!= ILfs.FALSE)
		{
			getFree().free(nbrList);
			return (ret);
		...
		nbr_nridges = new AtomicIntegerArray (oNoOfNbrs.get());
		...
		for (i = 0; i < oNoOfNbrs.get(); i++)
		{
			...
			ret = ridgeCount(first, nbrList.get(i), oMinutiae, 
	...
	**************************************************************************
	...
	#cat:               to the right of the primary point.
	   Input:
	      maxNbrs - maximum number of closest neighbors to be returned
	      firstMinutiaIndex    - index of the primary minutia point
	      oMinutiae - list of oMinutiae
	   Output:
	      oNbrList - points to list of detected closest neighbors
	      oNoOfNbrs    - points to number of neighbors returned
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	public int findNeighbors(AtomicIntegerArray oNbrList, AtomicInteger oNoOfNbrs, final int maxNbrs, 
		...
		while (secondMinutiaIndex < oMinutiae.get().getNum())
		{
			...
			if ((noOfNbrs.get() < maxNbrs) || (xdist2 < nbrSqrDists.get(lastNbr)))
			{
				...
				if ((ret = updateNbrDists(oNbrList, nbrSqrDists, noOfNbrs, 
					maxNbrs, firstMinutiaIndex, secondMinutiaIndex, oMinutiae)) < ILfs.FALSE)
				{
	...
	/*************************************************************************
	**************************************************************************
	#cat: updateNbrDists - Takes the current list of neighbors along with a
	...
	#cat:               squared distance to the primary point.
	   Input:
	      oNbrList - current list of nearest neighbor minutia indices
	      oNbrSqrDists - corresponding squared euclidean distance of each
	                 neighbor to the primary minutia point
	      oNoOfNbrs    - number of neighbors currently in the list
	      maxNbrs - maximum number of closest neighbors to be returned
	      firstMinutiaIndex    - index of the primary minutia point
	      secondMinutiaIndex   - index of the secondary (new neighbor) point
	      oMinutiae - list of oMinutiae
	   Output:
	      nbrList - updated list of nearest neighbor indices
	      nbrSqrDists - updated list of nearest neighbor distances
	      noOfNbrs    - number of neighbors in the update lists
	   Return Code:
	      Zero      - successful completion
	      Negative  - system error
	**************************************************************************/
	public int updateNbrDists(AtomicIntegerArray oNbrList, AtomicReferenceArray<Double> oNbrSqrDists, 
		...
		dist2 = getLfsUtil().squaredDistance(firstMinutia.getX(), firstMinutia.getY(), secondMinutia.getX(), secondMinutia.getY());
		...
		if ((oNoOfNbrs.get() < maxNbrs) || (dist2 < oNbrSqrDists.get(last_nbr)))
		{
			...
			/* specified location.                                    */
			if ((ret = insertNeighbor(pos, secondMinutiaIndex, dist2, oNbrList, oNbrSqrDists, oNoOfNbrs, maxNbrs)) < ILfs.FALSE)
			{
	...
	**************************************************************************
	...
	#cat:               stored values down and off the lists as necessary.
	   Input:
	      nbrListPos       - postions where values are to be inserted in lists
	      nbrIndex - index of minutia being inserted
	      nbrDist2 - squared distance of minutia to its primary point
	      oNbrList  - current list of nearest neighbor minutia indices
	      oNbrSqrDists - corresponding squared euclidean distance of each
	                  neighbor to the primary minutia point
	      oNoOfNbrs     - number of neighbors currently in the list
	...
	**************************************************************************/
	public int insertNeighbor(final int nbrListPos, final int nbrIndex, final double nbrDist2, 
		AtomicIntegerArray oNbrList, AtomicReferenceArray<Double> oNbrSqrDists, 
		AtomicInteger oNoOfNbrs, final int maxNbrs) {
		int currentNbrIndex;
	...
	**************************************************************************
	...
	#cat:               proceeding clockwise.
	   Input:
	      oNbrList - list of neighboring minutia indices
	      noOfNbrs    - number of neighbors in the list
	      firstMinutiaIndex    - the index of the primary minutia point
	      oMinutiae - list of oMinutiae
	   Output:
	      oNbrList - neighboring minutia indices in sorted order
	   Return Code:
	      Zero     - successful completion
	      Negative - system error
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: ridgeCount - Takes a pair of oMinutiae, and counts the number of
	#cat:               ridges crossed along the linear trajectory connecting
	#cat:               the 2 points in the image.
	   Input:
	      firstMinutiaIndex     - index of primary minutia
	      secondMinutiaIndex    - index of secondary (neighbor) minutia
	      oMinutiae  - list of oMinutiae
	      binarizedImageData     - binary image data (0==while & 1==black)
	      imageWidth        - width (in pixels) of image
	      imageHeight        - height (in pixels) of image
	...
	**************************************************************************/
	public int ridgeCount(final int firstMinutiaIndex, final int secondMinutiaIndex, 
		...
		int ridgeCount, ridgeStart, ridgeEnd;
		int prevpix, curpix;
		...
		int aSize = Math.max(Math.abs(secondMinutia.getX() - firstMinutia.getX()) + 2, Math.abs(secondMinutia.getY() - firstMinutia.getY()) + 2);
		xlist = new int[aSize]; 
		ylist= new int[aSize];
		if ((ret = getLine().linePoints(xlist, ylist, num, 
			firstMinutia.getX(), firstMinutia.getY(), secondMinutia.getX(), secondMinutia.getY())) != ILfs.FALSE)
		{
			return (ret);
		}
		...
		if (isShowLogs())
			LOGGER.info(String.format("RIDGE COUNT: %d,%d to %d,%d ", firstMinutia.getX(), firstMinutia.getY(), secondMinutia.getX(), secondMinutia.getY()));

		/* While not at the end of the trajectory ... */
		while (i.get() < num.get())
		{
			/* If 0-to-1 transition not found ... */
			if (findTransition(i, 0, 1, xlist, ylist, num.get(), 
			...
			if (isShowLogs())
				LOGGER.info(String.format(": RS %d,%d ", xlist[i.get()], ylist[i.get()]));

			/* If 1-to-0 transition not found ... */
			if (findTransition(i, 1, 0, xlist, ylist, num.get(), 
				binarizedImageData, imageWidth, imageHeight) == ILfs.FALSE)
			{
				/* Then we are done looking for ridges. */
				getFree().free(xlist);
			...
			if (isShowLogs())
				LOGGER.info(String.format("; RE %d,%d ", xlist[i.get()], ylist[i.get()]));
			...
			/* side of a ridge.                                          */
			ret = validateRidgeCrossing(ridgeStart, ridgeEnd, 
				xlist, ylist, num.get(), binarizedImageData, imageWidth, imageHeight, 
				lfsParams.getMaxRidgeSteps());
			/* If system error ... */
			if (ret < ILfs.FALSE)
			{
				getFree().free(xlist);
			...
			if (isShowLogs())
				LOGGER.info(String.format("; V%d ", ret));
	...
	**************************************************************************
	...
	#cat:               the pair was found (the index of the second pixel).
	   Input:
	      startPixel  - pointer to starting pixel index into trajectory
	      firstPixel  	- first pixel value in transition pair
	      secondPixel  	- second pixel value in transition pair
	...
	**************************************************************************/
	...
	/*************************************************************************
	**************************************************************************
	#cat: validateRidgeCrossing - Takes a pair of points, a ridge start
	...
	#cat:               ridge crossing.
	   Input:
	      ridgeStart - index into line trajectory of ridge start transition
	      ridgeEnd   - index into line trajectory of ridge end transition
	      xlist       - x-pixel coords of line trajectory
	      ylist       - y-pixel coords of line trajectory
	      num         - number of coords in line trajectory
	      binarizedImageData       - binary image data (0==while & 1==black)
	      imageWidth          - width (in pixels) of image
	      imageHeight          - height (in pixels) of image
	      maxRidgeSteps  - number of steps taken in search in both
	                         scan directions
	   Return Code:
	...
	**************************************************************************/
	public int validateRidgeCrossing(final int ridgeStart, final int ridgeEnd, 
		...
		AtomicInteger featureX = new AtomicInteger(0), featureY = new AtomicInteger(0), 
			edgeX = new AtomicInteger(0), edgeY = new AtomicInteger(0);
		...
		contour = getContour().traceContour(ret, noOfContour, maxRidgeSteps, 
			xlist[ridgeStart - 1], ylist[ridgeStart - 1], featureX.get(), featureY.get(), edgeX.get(), edgeY.get(), 
